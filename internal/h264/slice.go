...
func ParseSliceHeader(nal NALUnit, ppsList *[maxPPSCount]*PPS) (*SliceHeader, error) {
	sh := &SliceHeader{}
	_, err := parseSliceHeaderWithPayloadInto(nal, ppsList, sh)
	return sh, err
}

func parseSliceHeaderWithPayload(nal NALUnit, ppsList *[maxPPSCount]*PPS) (*SliceHeader, bitReader, error) {
	sh := &SliceHeader{}
	gb, err := parseSliceHeaderWithPayloadInto(nal, ppsList, sh)
	if err != nil {
		return nil, gb, err
	}
	return sh, gb, nil
...
func parseSliceHeaderWithPayloadInto(nal NALUnit, ppsList *[maxPPSCount]*PPS, sh *SliceHeader) (bitReader, error) {
	if sh == nil || (nal.Type != NALSlice && nal.Type != NALIDRSlice) {
		return bitReader{}, ErrInvalidData
	}
