...
func (m *macroblockTables) decodeCABACFrameSliceHigh(src cabacSyntaxSource, dst *h264PicturePlanesHigh, sh *SliceHeader, in h264FrameSliceDecodeInputHigh) (h264FrameSliceDecodeResult, error) {
	...
	for {
		...
		if cur.FrameMBAFF {
			...
			var bottomWork frameMacroblockDecodeWork
			bottomMB, err := m.decodeCABACFrameSliceMacroblockWithDirectWorkGuard(src, sh, &state, bottom.MBXY, in.SliceNum, in.Direct, &bottomWork, sh.SliceTypeNoS == PictureTypeB)
			if err != nil {
...
func isHighChromaFrameDeblockScope(sh *SliceHeader) bool {
	...
	if sh.SliceTypeNoS != PictureTypeI && sh.SliceTypeNoS != PictureTypeP && (sh.SliceTypeNoS != PictureTypeB || sh.SPS.BitDepthLuma != 9) {
		return false
	}
	...
	case PictureTypeI:
		if sh.SPS != nil && sh.SPS.BitDepthLuma != 9 && sh.SPS.BitDepthLuma != 10 {
			...
			if sh.SPS.BitDepthLuma == 12 {
				...
				if mbType == MBTypeIntra16x16 && cbp == 0x2f && (cbpTable == 0x16f || cbpTable == 0xf02f) {
					return nil
				}
