// SPDX-License-Identifier: LGPL-2.1-or-later
//
// Source-shaped frame-MB motion-compensation call-site helpers from FFmpeg
// n8.0.1 libavcodec/h264_mb.c mc_dir_part/mc_part_std and
// libavcodec/h264_mc_template.c hl_motion.

package h264

type h264PicturePlanes struct {
	Y, Cb, Cr        []uint8
	LumaStride       int
	ChromaStride     int
	MBWidth          int
	MBHeight         int
	ChromaFormatIDC  int
	PictureStructure int32
	// trustedLayout is set only on internal DPB views after their backing frame
	// and optional field projection have already established valid geometry.
	trustedLayout bool
}

type h264MotionCompScratch struct {
	Y, Cb, Cr []uint8
	Edge      []uint8
}

func h264HLMotionFrame(dst *h264PicturePlanes, refs [2][]*h264PicturePlanes, cache *macroblockMotionCache, mbType uint32, subMBType [4]uint32, mbX int, mbY int, listCount int) error {
	return h264HLMotionFrameCore(dst, &refs, cache, mbType, subMBType, mbX, mbY, listCount, nil, nil, mbY, false)
}

func h264HLMotionFrameWithScratch(dst *h264PicturePlanes, refs [2][]*h264PicturePlanes, cache *macroblockMotionCache, mbType uint32, subMBType [4]uint32, mbX int, mbY int, listCount int, scratch *h264MotionCompScratch) error {
	return h264HLMotionFrameCore(dst, &refs, cache, mbType, subMBType, mbX, mbY, listCount, nil, scratch, mbY, false)
}

func h264HLMotionFrameWithScratchTrusted(dst *h264PicturePlanes, refs *[2][]*h264PicturePlanes, cache *macroblockMotionCache, mbType uint32, subMBType [4]uint32, mbX int, mbY int, listCount int, scratch *h264MotionCompScratch) error {
	return h264HLMotionFrameCore(dst, refs, cache, mbType, subMBType, mbX, mbY, listCount, nil, scratch, mbY, true)
}

func h264HLMotionFrameWeighted(dst *h264PicturePlanes, refs [2][]*h264PicturePlanes, cache *macroblockMotionCache, mbType uint32, subMBType [4]uint32, mbX int, mbY int, listCount int, pwt *PredWeightTable, scratch *h264MotionCompScratch) error {
	return h264HLMotionFrameWeightedWithWeightY(dst, refs, cache, mbType, subMBType, mbX, mbY, listCount, mbY, pwt, scratch)
}

func h264HLMotionFrameWeightedWithWeightY(dst *h264PicturePlanes, refs [2][]*h264PicturePlanes, cache *macroblockMotionCache, mbType uint32, subMBType [4]uint32, mbX int, mbY int, listCount int, weightMBY int, pwt *PredWeightTable, scratch *h264MotionCompScratch) error {
	if pwt == nil {
		return ErrInvalidData
	}
	return h264HLMotionFrameCore(dst, &refs, cache, mbType, subMBType, mbX, mbY, listCount, pwt, scratch, weightMBY, false)
}

func h264HLMotionFrameWeightedWithWeightYTrusted(dst *h264PicturePlanes, refs *[2][]*h264PicturePlanes, cache *macroblockMotionCache, mbType uint32, subMBType [4]uint32, mbX int, mbY int, listCount int, weightMBY int, pwt *PredWeightTable, scratch *h264MotionCompScratch) error {
	if pwt == nil {
		return ErrInvalidData
	}
	return h264HLMotionFrameCore(dst, refs, cache, mbType, subMBType, mbX, mbY, listCount, pwt, scratch, weightMBY, true)
}

func h264HLMotionFrameCore(dst *h264PicturePlanes, refs *[2][]*h264PicturePlanes, cache *macroblockMotionCache, mbType uint32, subMBType [4]uint32, mbX int, mbY int, listCount int, pwt *PredWeightTable, scratch *h264MotionCompScratch, weightMBY int, trustedDst bool) error {
	if dst == nil || refs == nil || cache == nil || mbX < 0 || mbY < 0 || listCount < 0 || listCount > 2 {
		return ErrInvalidData
	}
	if !trustedDst {
		if err := dst.validate(); err != nil {
			return err
		}
	}
	if mbX >= dst.MBWidth || mbY >= dst.MBHeight || !isInter(mbType) {
		return ErrInvalidData
	}

	if is16x16(mbType) {
		if trustedDst && dst.ChromaFormatIDC == 1 && dst.PictureStructure == PictureFrame {
			list0 := isDir(mbType, 0, 0)
			list1 := isDir(mbType, 0, 1)
			if !h264MCPartUsesWeighted(pwt, cache, mbType, 0, list0, list1, weightMBY) {
				handled, err := h264MC16x16Progressive420StdTrusted(dst, refs, cache, mbX, mbY, listCount, list0, list1, scratch)
				if handled {
					return err
				}
			}
		}
		return h264MCPartFrameValidated(dst, refs, cache, mbX, mbY, mbType, 0, 0, true, 16, 0, 0, 0, 16, 8, 16, listCount, pwt, scratch, weightMBY)
	}
	if is16x8(mbType) {
		if err := h264MCPartFrameValidated(dst, refs, cache, mbX, mbY, mbType, 0, 0, false, 8, 8, 0, 0, 8, 8, 16, listCount, pwt, scratch, weightMBY); err != nil {
			return err
		}
		return h264MCPartFrameValidated(dst, refs, cache, mbX, mbY, mbType, 1, 8, false, 8, 8, 0, 4, 8, 8, 16, listCount, pwt, scratch, weightMBY)
	}
	if is8x16(mbType) {
		delta := 8 * dst.LumaStride
		if err := h264MCPartFrameValidated(dst, refs, cache, mbX, mbY, mbType, 0, 0, false, 16, delta, 0, 0, 8, 4, 8, listCount, pwt, scratch, weightMBY); err != nil {
			return err
		}
		return h264MCPartFrameValidated(dst, refs, cache, mbX, mbY, mbType, 1, 4, false, 16, delta, 4, 0, 8, 4, 8, listCount, pwt, scratch, weightMBY)
	}
	if !is8x8(mbType) {
		return ErrUnsupported
	}

	for i := 0; i < 4; i++ {
		subType := subMBType[i]
		partType := subType | (mbType & MBTypeInterlaced)
		n := 4 * i
		xOffset := (i & 1) << 2
		yOffset := (i & 2) << 1

		if isSub8x8(subType) {
			if err := h264MCPartFrameValidated(dst, refs, cache, mbX, mbY, partType, 0, n, true, 8, 0, xOffset, yOffset, 8, 4, 8, listCount, pwt, scratch, weightMBY); err != nil {
				return err
			}
		} else if isSub8x4(subType) {
			if err := h264MCPartFrameValidated(dst, refs, cache, mbX, mbY, partType, 0, n, false, 4, 4, xOffset, yOffset, 4, 4, 8, listCount, pwt, scratch, weightMBY); err != nil {
				return err
			}
			if err := h264MCPartFrameValidated(dst, refs, cache, mbX, mbY, partType, 0, n+2, false, 4, 4, xOffset, yOffset+2, 4, 4, 8, listCount, pwt, scratch, weightMBY); err != nil {
				return err
			}
		} else if isSub4x8(subType) {
			delta := 4 * dst.LumaStride
			if err := h264MCPartFrameValidated(dst, refs, cache, mbX, mbY, partType, 0, n, false, 8, delta, xOffset, yOffset, 4, 2, 4, listCount, pwt, scratch, weightMBY); err != nil {
				return err
			}
			if err := h264MCPartFrameValidated(dst, refs, cache, mbX, mbY, partType, 0, n+1, false, 8, delta, xOffset+2, yOffset, 4, 2, 4, listCount, pwt, scratch, weightMBY); err != nil {
				return err
			}
		} else if isSub4x4(subType) {
			for j := 0; j < 4; j++ {
				subXOffset := xOffset + 2*(j&1)
				subYOffset := yOffset + (j & 2)
				if err := h264MCPartFrameValidated(dst, refs, cache, mbX, mbY, partType, 0, n+j, true, 4, 0, subXOffset, subYOffset, 4, 2, 4, listCount, pwt, scratch, weightMBY); err != nil {
					return err
				}
			}
		} else {
			return ErrUnsupported
		}
	}
	return nil
}

// h264MC16x16Progressive420StdTrusted fuses the dominant frame-picture
// 16x16 standard prediction shape after the enclosing decoder has validated
// the destination, DPB, macroblock geometry, and motion cache.
func h264MC16x16Progressive420StdTrusted(dst *h264PicturePlanes, refs *[2][]*h264PicturePlanes, cache *macroblockMotionCache, mbX int, mbY int, listCount int, list0 bool, list1 bool, scratch *h264MotionCompScratch) (bool, error) {
	if dst == nil || refs == nil || cache == nil || dst.ChromaFormatIDC != 1 || dst.PictureStructure != PictureFrame {
		return false, nil
	}
	if (!list0 && !list1) || (list0 && listCount < 1) || (list1 && listCount < 2) {
		if !list0 && !list1 {
			return true, nil
		}
		return true, ErrInvalidData
	}

	var ref0 *h264PicturePlanes
	var ref1 *h264PicturePlanes
	if list0 {
		var err error
		ref0, err = h264MCReference(refs, cache, 0, 0)
		if err != nil {
			return true, err
		}
		if !h264MC16x16Progressive420RefMatches(dst, ref0) {
			return false, nil
		}
		if !ref0.trustedLayout {
			if err := ref0.validate(); err != nil {
				return true, err
			}
		}
	}
	if list1 {
		var err error
		ref1, err = h264MCReference(refs, cache, 1, 0)
		if err != nil {
			return true, err
		}
		if !h264MC16x16Progressive420RefMatches(dst, ref1) {
			return false, nil
		}
		if !ref1.trustedLayout {
			if err := ref1.validate(); err != nil {
				return true, err
			}
		}
	}

	dstY := mbY*16*dst.LumaStride + mbX*16
	dstC := mbY*8*dst.ChromaStride + mbX*8
	if list0 {
		if err := h264MCDir16x16Progressive420StdTrusted(dst, ref0, cache, 0, mbX, mbY, dstY, dstC, false, scratch); err != nil {
			return true, err
		}
	}
	if list1 {
		if err := h264MCDir16x16Progressive420StdTrusted(dst, ref1, cache, 1, mbX, mbY, dstY, dstC, list0, scratch); err != nil {
			return true, err
		}
	}
	return true, nil
}

func h264MC16x16Progressive420RefMatches(dst *h264PicturePlanes, ref *h264PicturePlanes) bool {
	return ref != nil && ref.PictureStructure == PictureFrame && ref.ChromaFormatIDC == 1 && ref.LumaStride == dst.LumaStride && ref.ChromaStride == dst.ChromaStride && ref.MBWidth == dst.MBWidth && ref.MBHeight == dst.MBHeight
}

func h264MCDir16x16Progressive420StdTrusted(dst *h264PicturePlanes, ref *h264PicturePlanes, cache *macroblockMotionCache, list int, mbX int, mbY int, dstY int, dstC int, avg bool, scratch *h264MotionCompScratch) error {
	mv := cache.MV[list][h264Scan8[0]]
	mx := int(mv[0]) + 8*mbX*8
	my := int(mv[1]) + 8*mbY*8
	fullMx := mx >> 2
	fullMy := my >> 2
	extraWidth := 0
	extraHeight := 0
	if mx&7 != 0 {
		extraWidth = -3
	}
	if my&7 != 0 {
		extraHeight = -3
	}
	if fullMx < -extraWidth || fullMy < -extraHeight || fullMx+16 > ref.MBWidth*16+extraWidth || fullMy+16 > ref.MBHeight*16+extraHeight {
		return h264MCDirPartFramePlanesCore(dst, ref, cache, 0, true, 16, 0, list, dstY, dstC, dstC, 8*mbX, 8*mbY, 16, 8, avg, scratch, true)
	}

	srcY := fullMx + fullMy*ref.LumaStride
	h264QpelMCStridesKernel(dst.Y, dstY, dst.LumaStride, ref.Y, srcY, ref.LumaStride, 16, int32(mx&3), int32(my&3), avg)

	chromaX := mx & 7
	chromaY := my & 7
	chromaBaseX := mx >> 3
	chromaBaseY := my >> 3
	extraChromaWidth := 0
	extraChromaHeight := 0
	if chromaX != 0 {
		extraChromaWidth = -1
	}
	if chromaY != 0 {
		extraChromaHeight = -1
	}
	if chromaBaseX >= 0 && chromaBaseY >= 0 && chromaBaseX+8 <= ref.MBWidth*8+extraChromaWidth && chromaBaseY+8 <= ref.MBHeight*8+extraChromaHeight {
		srcC := chromaBaseX + chromaBaseY*ref.ChromaStride
		h264ChromaMCDualStridesKernel(dst.Cb[dstC:], dst.Cr[dstC:], ref.Cb[srcC:], ref.Cr[srcC:], dst.ChromaStride, ref.ChromaStride, 8, int32(chromaX), int32(chromaY), 8, avg)
		return nil
	}
	return h264MCDirPartFrameProgressive420ChromaCore(dst, ref, mx, my, 16, dstC, dstC, 8, avg, scratch, true)
}

func h264MCPartFrame(dst *h264PicturePlanes, refs [2][]*h264PicturePlanes, cache *macroblockMotionCache, mbX int, mbY int, mbType uint32, part int, n int, square bool, height int, delta int, xOffset int, yOffset int, qpelSize int, chromaWidth int, lumaWeightWidth int, listCount int, pwt *PredWeightTable, scratch *h264MotionCompScratch, weightMBY int) error {
	list0 := isDir(mbType, part, 0)
	list1 := isDir(mbType, part, 1)
	if h264MCPartUsesWeighted(pwt, cache, mbType, n, list0, list1, weightMBY) {
		return h264MCPartFrameWeighted(dst, refs, cache, mbX, mbY, mbType, part, n, square, height, delta, xOffset, yOffset, qpelSize, chromaWidth, lumaWeightWidth, listCount, pwt, scratch, weightMBY)
	}
	return h264MCPartFrameStd(dst, refs, cache, mbX, mbY, mbType, part, n, square, height, delta, xOffset, yOffset, qpelSize, chromaWidth, listCount, scratch)
}

func h264MCPartFrameValidated(dst *h264PicturePlanes, refs *[2][]*h264PicturePlanes, cache *macroblockMotionCache, mbX int, mbY int, mbType uint32, part int, n int, square bool, height int, delta int, xOffset int, yOffset int, qpelSize int, chromaWidth int, lumaWeightWidth int, listCount int, pwt *PredWeightTable, scratch *h264MotionCompScratch, weightMBY int) error {
	list0 := isDir(mbType, part, 0)
	list1 := isDir(mbType, part, 1)
	if h264MCPartUsesWeighted(pwt, cache, mbType, n, list0, list1, weightMBY) {
		return h264MCPartFrameWeightedCore(dst, refs, cache, mbX, mbY, mbType, part, n, square, height, delta, xOffset, yOffset, qpelSize, chromaWidth, lumaWeightWidth, listCount, pwt, scratch, weightMBY, true)
	}
	return h264MCPartFrameStdCore(dst, refs, cache, mbX, mbY, mbType, part, n, square, height, delta, xOffset, yOffset, qpelSize, chromaWidth, listCount, scratch, true)
}

func h264ImplicitWeightIndexes(mbType uint32, refn0 int, refn1 int, mbY int) (int, int) {
	if mbType&MBTypeInterlaced != 0 {
		parity := mbY & 1
		return (h264ImplicitMBAFFWeightOffset + refn0) ^ parity, (h264ImplicitMBAFFWeightOffset + refn1) ^ parity
	}
	return refn0, refn1
}

func h264MCPartUsesWeighted(pwt *PredWeightTable, cache *macroblockMotionCache, mbType uint32, n int, list0 bool, list1 bool, mbY int) bool {
	if pwt == nil || cache == nil || n < 0 || n >= 16 {
		return false
	}
	if pwt.UseWeight == 1 {
		return true
	}
	if pwt.UseWeight != 2 || !list0 || !list1 {
		return false
	}
	refn0 := cache.Ref[0][h264Scan8[n]]
	refn1 := cache.Ref[1][h264Scan8[n]]
	weightRef0, weightRef1 := h264ImplicitWeightIndexes(mbType, int(refn0), int(refn1), mbY)
	if refn0 < 0 || refn1 < 0 || weightRef0 >= len(pwt.ImplicitWeight) || weightRef1 >= len(pwt.ImplicitWeight[0]) {
		return false
	}
	return pwt.ImplicitWeight[weightRef0][weightRef1][mbY&1] != 32
}

func h264MCPartFrameStd(dst *h264PicturePlanes, refs [2][]*h264PicturePlanes, cache *macroblockMotionCache, mbX int, mbY int, mbType uint32, part int, n int, square bool, height int, delta int, xOffset int, yOffset int, qpelSize int, chromaWidth int, listCount int, scratch *h264MotionCompScratch) error {
	return h264MCPartFrameStdCore(dst, &refs, cache, mbX, mbY, mbType, part, n, square, height, delta, xOffset, yOffset, qpelSize, chromaWidth, listCount, scratch, false)
}

func h264MCPartFrameStdCore(dst *h264PicturePlanes, refs *[2][]*h264PicturePlanes, cache *macroblockMotionCache, mbX int, mbY int, mbType uint32, part int, n int, square bool, height int, delta int, xOffset int, yOffset int, qpelSize int, chromaWidth int, listCount int, scratch *h264MotionCompScratch, dstValidated bool) error {
	list0 := isDir(mbType, part, 0)
	list1 := isDir(mbType, part, 1)
	if (!list0 && !list1) || qpelSize <= 0 {
		return nil
	}
	if list0 && listCount < 1 || list1 && listCount < 2 {
		return ErrInvalidData
	}
	dstY, dstCb, dstCr, err := h264MBDestPartOffsets(dst, mbX, mbY, xOffset, yOffset)
	if err != nil {
		return err
	}
	srcXOffset := xOffset + 8*mbX
	srcYOffset := yOffset + 8*mbY
	avg := false

	if list0 {
		ref, err := h264MCReference(refs, cache, 0, n)
		if err != nil {
			return err
		}
		if err := h264MCDirPartFrameMaybeValidated(dst, ref, cache, n, square, height, delta, 0, dstY, dstCb, dstCr, srcXOffset, srcYOffset, qpelSize, chromaWidth, avg, scratch, dstValidated); err != nil {
			return err
		}
		avg = true
	}
	if list1 {
		ref, err := h264MCReference(refs, cache, 1, n)
		if err != nil {
			return err
		}
		if err := h264MCDirPartFrameMaybeValidated(dst, ref, cache, n, square, height, delta, 1, dstY, dstCb, dstCr, srcXOffset, srcYOffset, qpelSize, chromaWidth, avg, scratch, dstValidated); err != nil {
			return err
		}
	}
	return nil
}

func h264MCPartFrameWeighted(dst *h264PicturePlanes, refs [2][]*h264PicturePlanes, cache *macroblockMotionCache, mbX int, mbY int, mbType uint32, part int, n int, square bool, height int, delta int, xOffset int, yOffset int, qpelSize int, chromaWidth int, lumaWeightWidth int, listCount int, pwt *PredWeightTable, scratch *h264MotionCompScratch, weightMBY int) error {
	return h264MCPartFrameWeightedCore(dst, &refs, cache, mbX, mbY, mbType, part, n, square, height, delta, xOffset, yOffset, qpelSize, chromaWidth, lumaWeightWidth, listCount, pwt, scratch, weightMBY, false)
}

func h264MCPartFrameWeightedCore(dst *h264PicturePlanes, refs *[2][]*h264PicturePlanes, cache *macroblockMotionCache, mbX int, mbY int, mbType uint32, part int, n int, square bool, height int, delta int, xOffset int, yOffset int, qpelSize int, chromaWidth int, lumaWeightWidth int, listCount int, pwt *PredWeightTable, scratch *h264MotionCompScratch, weightMBY int, dstValidated bool) error {
	list0 := isDir(mbType, part, 0)
	list1 := isDir(mbType, part, 1)
	if (!list0 && !list1) || qpelSize <= 0 || lumaWeightWidth <= 0 || pwt == nil {
		return nil
	}
	if list0 && listCount < 1 || list1 && listCount < 2 {
		return ErrInvalidData
	}
	dstY, dstCb, dstCr, err := h264MBDestPartOffsets(dst, mbX, mbY, xOffset, yOffset)
	if err != nil {
		return err
	}
	srcXOffset := xOffset + 8*mbX
	srcYOffset := yOffset + 8*mbY
	chromaHeight, chromaWeightWidth, err := h264ChromaWeightGeometry(dst.ChromaFormatIDC, height, chromaWidth, lumaWeightWidth)
	if err != nil {
		return err
	}

	if list0 && list1 {
		ref0, err := h264MCReference(refs, cache, 0, n)
		if err != nil {
			return err
		}
		ref1, err := h264MCReference(refs, cache, 1, n)
		if err != nil {
			return err
		}
		if scratch == nil || !scratch.valid(dst, height, lumaWeightWidth, chromaHeight, chromaWeightWidth) {
			return ErrInvalidData
		}
		if err := h264MCDirPartFrameMaybeValidated(dst, ref0, cache, n, square, height, delta, 0, dstY, dstCb, dstCr, srcXOffset, srcYOffset, qpelSize, chromaWidth, false, scratch, dstValidated); err != nil {
			return err
		}
		if err := h264MCDirPartFramePlanesMaybeValidated(scratch.Y, scratch.Cb, scratch.Cr, dst.LumaStride, dst.ChromaStride, dst.ChromaFormatIDC, dst.PictureStructure, ref1, cache, n, square, height, delta, 1, 0, 0, 0, srcXOffset, srcYOffset, qpelSize, chromaWidth, false, scratch, dstValidated); err != nil {
			return err
		}
		refn0 := int(cache.Ref[0][h264Scan8[n]])
		refn1 := int(cache.Ref[1][h264Scan8[n]])
		if refn0 < 0 || refn1 < 0 || refn0 >= len(pwt.LumaWeight) || refn1 >= len(pwt.LumaWeight) {
			return ErrInvalidData
		}
		if pwt.UseWeight == 2 {
			weightRef0, weightRef1 := h264ImplicitWeightIndexes(mbType, refn0, refn1, weightMBY)
			if weightRef0 >= len(pwt.ImplicitWeight) || weightRef1 >= len(pwt.ImplicitWeight[0]) {
				return ErrInvalidData
			}
			weight0 := int(pwt.ImplicitWeight[weightRef0][weightRef1][weightMBY&1])
			weight1 := 64 - weight0
			if err := h264BiweightPixels(dst.Y[dstY:], scratch.Y, dst.LumaStride, height, 5, weight0, weight1, 0, lumaWeightWidth); err != nil {
				return err
			}
			if dst.ChromaFormatIDC != 0 {
				if err := h264BiweightPixels(dst.Cb[dstCb:], scratch.Cb, dst.ChromaStride, chromaHeight, 5, weight0, weight1, 0, chromaWeightWidth); err != nil {
					return err
				}
				return h264BiweightPixels(dst.Cr[dstCr:], scratch.Cr, dst.ChromaStride, chromaHeight, 5, weight0, weight1, 0, chromaWeightWidth)
			}
			return nil
		}
		if err := h264BiweightPixels(dst.Y[dstY:], scratch.Y, dst.LumaStride, height, int(pwt.LumaLog2WeightDenom), int(pwt.LumaWeight[refn0][0][0]), int(pwt.LumaWeight[refn1][1][0]), int(pwt.LumaWeight[refn0][0][1]+pwt.LumaWeight[refn1][1][1]), lumaWeightWidth); err != nil {
			return err
		}
		if dst.ChromaFormatIDC != 0 {
			if err := h264BiweightPixels(dst.Cb[dstCb:], scratch.Cb, dst.ChromaStride, chromaHeight, int(pwt.ChromaLog2WeightDenom), int(pwt.ChromaWeight[refn0][0][0][0]), int(pwt.ChromaWeight[refn1][1][0][0]), int(pwt.ChromaWeight[refn0][0][0][1]+pwt.ChromaWeight[refn1][1][0][1]), chromaWeightWidth); err != nil {
				return err
			}
			return h264BiweightPixels(dst.Cr[dstCr:], scratch.Cr, dst.ChromaStride, chromaHeight, int(pwt.ChromaLog2WeightDenom), int(pwt.ChromaWeight[refn0][0][1][0]), int(pwt.ChromaWeight[refn1][1][1][0]), int(pwt.ChromaWeight[refn0][0][1][1]+pwt.ChromaWeight[refn1][1][1][1]), chromaWeightWidth)
		}
		return nil
	}

	list := 0
	if list1 {
		list = 1
	}
	ref, err := h264MCReference(refs, cache, list, n)
	if err != nil {
		return err
	}
	refn := int(cache.Ref[list][h264Scan8[n]])
	if refn < 0 || refn >= len(pwt.LumaWeight) {
		return ErrInvalidData
	}
	if err := h264MCDirPartFrameMaybeValidated(dst, ref, cache, n, square, height, delta, list, dstY, dstCb, dstCr, srcXOffset, srcYOffset, qpelSize, chromaWidth, false, scratch, dstValidated); err != nil {
		return err
	}
	if err := h264WeightPixels(dst.Y[dstY:], dst.LumaStride, height, int(pwt.LumaLog2WeightDenom), int(pwt.LumaWeight[refn][list][0]), int(pwt.LumaWeight[refn][list][1]), lumaWeightWidth); err != nil {
		return err
	}
	if dst.ChromaFormatIDC != 0 && pwt.UseWeightChroma != 0 {
		if err := h264WeightPixels(dst.Cb[dstCb:], dst.ChromaStride, chromaHeight, int(pwt.ChromaLog2WeightDenom), int(pwt.ChromaWeight[refn][list][0][0]), int(pwt.ChromaWeight[refn][list][0][1]), chromaWeightWidth); err != nil {
			return err
		}
		return h264WeightPixels(dst.Cr[dstCr:], dst.ChromaStride, chromaHeight, int(pwt.ChromaLog2WeightDenom), int(pwt.ChromaWeight[refn][list][1][0]), int(pwt.ChromaWeight[refn][list][1][1]), chromaWeightWidth)
	}
	return nil
}

func h264ChromaWeightGeometry(chromaFormatIDC int, height int, chromaWidth int, lumaWeightWidth int) (int, int, error) {
	switch chromaFormatIDC {
	case 0:
		return 0, 0, nil
	case 1:
		return height >> 1, chromaWidth, nil
	case 2:
		return height, chromaWidth, nil
	case 3:
		return height, lumaWeightWidth, nil
	default:
		return 0, 0, ErrInvalidData
	}
}

func (s *h264MotionCompScratch) valid(dst *h264PicturePlanes, lumaHeight int, lumaWidth int, chromaHeight int, chromaWidth int) bool {
	if s == nil || dst == nil {
		return false
	}
	if !h264PlaneHas(s.Y, dst.LumaStride, lumaHeight, lumaWidth) {
		return false
	}
	if dst.ChromaFormatIDC == 0 {
		return true
	}
	return h264PlaneHas(s.Cb, dst.ChromaStride, chromaHeight, chromaWidth) &&
		h264PlaneHas(s.Cr, dst.ChromaStride, chromaHeight, chromaWidth)
}

func h264PlaneHas(p []uint8, stride int, height int, width int) bool {
	if stride <= 0 || height < 0 || width < 0 {
		return false
	}
	if height == 0 {
		return true
	}
	if width == 0 || stride < width {
		return false
	}
	need, err := h264PlaneSpanLength(stride, height, width)
	return err == nil && len(p) >= need
}

func h264EdgeStride(stride int, blockW int) int {
	if stride < blockW {
		return blockW
	}
	return stride
}

func h264EdgeScratchSize(stride int, blockW int, blockH int) int {
	edgeStride := h264EdgeStride(stride, blockW)
	return (blockH-1)*edgeStride + blockW
}

func h264EdgeScratch(s *h264MotionCompScratch, stride int, blockW int, blockH int) ([]uint8, int, error) {
	if s == nil || stride <= 0 || blockW <= 0 || blockH <= 0 {
		return nil, 0, ErrInvalidData
	}
	edgeStride := h264EdgeStride(stride, blockW)
	needed, ok := checkedH264EdgeScratchSize(stride, blockW, blockH)
	if !ok {
		return nil, 0, ErrInvalidData
	}
	if len(s.Edge) < needed {
		return nil, 0, ErrInvalidData
	}
	return s.Edge, edgeStride, nil
}

func h264EmulatedEdgeMC(buf []uint8, bufOffset int, bufStride int, src []uint8, srcStride int, blockW int, blockH int, srcX int, srcY int, width int, height int) error {
	if bufOffset < 0 || bufStride <= 0 || srcStride <= 0 || blockW <= 0 || blockH <= 0 || width < 0 || height < 0 {
		return ErrInvalidData
	}
	if width == 0 || height == 0 {
		return nil
	}
	if bufStride < blockW || srcStride < width {
		return ErrInvalidData
	}
	bufEnd, err := h264PlaneSpanEnd(bufOffset, bufStride, blockH, blockW)
	if err != nil {
		return ErrInvalidData
	}
	srcNeed, err := h264PlaneSpanLength(srcStride, height, width)
	if err != nil {
		return ErrInvalidData
	}
	if bufEnd > len(buf) || len(src) < srcNeed {
		return ErrInvalidData
	}
	if srcY >= height {
		srcY = height - 1
	} else if srcY <= -blockH {
		srcY = 1 - blockH
	}
	if srcX >= width {
		srcX = width - 1
	} else if srcX <= -blockW {
		srcX = 1 - blockW
	}
	startY := max(0, -srcY)
	startX := max(0, -srcX)
	endY := min(blockH, height-srcY)
	endX := min(blockW, width-srcX)
	copyW := endX - startX
	srcX += startX
	if h264EmulatedEdgeMCASMEnabled && (blockW == 21 || blockW == 9) {
		srcOffset := (srcY+startY)*srcStride + srcX
		h264EmulatedEdgeMCRowsASM(&buf[bufOffset], bufStride, &src[srcOffset], srcStride, blockW, blockH, startX, startY, endX, endY)
		return nil
	}
	for y := 0; y < blockH; y++ {
		sy := y
		if sy < startY {
			sy = startY
		} else if sy >= endY {
			sy = endY - 1
		}
		dstRow := bufOffset + y*bufStride
		copy(buf[dstRow+startX:dstRow+startX+copyW], src[(srcY+sy)*srcStride+srcX:(srcY+sy)*srcStride+srcX+copyW])
		if startX > 0 {
			v := buf[dstRow+startX]
			for x := 0; x < startX; x++ {
				buf[dstRow+x] = v
			}
		}
		if endX < blockW {
			v := buf[dstRow+endX-1]
			for x := endX; x < blockW; x++ {
				buf[dstRow+x] = v
			}
		}
	}
	return nil
}

func h264MCDirPartFrame(dst *h264PicturePlanes, ref *h264PicturePlanes, cache *macroblockMotionCache, n int, square bool, height int, delta int, list int, dstY int, dstCb int, dstCr int, srcXOffset int, srcYOffset int, qpelSize int, chromaWidth int, avg bool, scratch *h264MotionCompScratch) error {
	if dst == nil || ref == nil || cache == nil || n < 0 || n >= 16 || list < 0 || list > 1 || height <= 0 || delta < 0 {
		return ErrInvalidData
	}
	if err := h264CheckMotionPlanePair(dst, ref); err != nil {
		return err
	}
	return h264MCDirPartFrameChecked(dst, ref, cache, n, square, height, delta, list, dstY, dstCb, dstCr, srcXOffset, srcYOffset, qpelSize, chromaWidth, avg, scratch, false)
}

func h264MCDirPartFramePlanes(dstYPlane []uint8, dstCbPlane []uint8, dstCrPlane []uint8, dstLumaStride int, dstChromaStride int, chromaFormatIDC int, dstPictureStructure int32, ref *h264PicturePlanes, cache *macroblockMotionCache, n int, square bool, height int, delta int, list int, dstY int, dstCb int, dstCr int, srcXOffset int, srcYOffset int, qpelSize int, chromaWidth int, avg bool, scratch *h264MotionCompScratch) error {
	if err := h264CheckMCDirPartFramePlanesArgs(dstLumaStride, dstChromaStride, chromaFormatIDC, ref, cache, n, height, delta, list); err != nil {
		return err
	}
	if !ref.trustedLayout {
		if err := ref.validate(); err != nil {
			return err
		}
	}
	return h264MCDirPartFramePlanesChecked(dstYPlane, dstCbPlane, dstCrPlane, dstLumaStride, dstChromaStride, chromaFormatIDC, dstPictureStructure, ref, cache, n, square, height, delta, list, dstY, dstCb, dstCr, srcXOffset, srcYOffset, qpelSize, chromaWidth, avg, scratch, false)
}

func h264MCDirPartFrameMaybeValidated(dst *h264PicturePlanes, ref *h264PicturePlanes, cache *macroblockMotionCache, n int, square bool, height int, delta int, list int, dstY int, dstCb int, dstCr int, srcXOffset int, srcYOffset int, qpelSize int, chromaWidth int, avg bool, scratch *h264MotionCompScratch, dstValidated bool) error {
	if !dstValidated {
		return h264MCDirPartFrame(dst, ref, cache, n, square, height, delta, list, dstY, dstCb, dstCr, srcXOffset, srcYOffset, qpelSize, chromaWidth, avg, scratch)
	}
	if ref == nil {
		return ErrInvalidData
	}
	if !ref.trustedLayout {
		if err := ref.validate(); err != nil {
			return err
		}
	}
	// The trusted reconstruction entrypoint has already validated the current
	// picture, macroblock geometry, cache indexes, and DPB reference layout.
	// Go directly to the kernel core instead of repeating those checks for every
	// partition in the macroblock.
	return h264MCDirPartFramePlanesCore(dst, ref, cache, n, square, height, delta, list, dstY, dstCb, dstCr, srcXOffset, srcYOffset, qpelSize, chromaWidth, avg, scratch, true)
}

func h264MCDirPartFrameChecked(dst *h264PicturePlanes, ref *h264PicturePlanes, cache *macroblockMotionCache, n int, square bool, height int, delta int, list int, dstY int, dstCb int, dstCr int, srcXOffset int, srcYOffset int, qpelSize int, chromaWidth int, avg bool, scratch *h264MotionCompScratch, trustedKernelArgs bool) error {
	if dst == nil || ref == nil || cache == nil || n < 0 || n >= 16 || list < 0 || list > 1 || height <= 0 || delta < 0 {
		return ErrInvalidData
	}
	if err := h264CheckMotionPlanePairShape(dst, ref); err != nil {
		return err
	}
	return h264MCDirPartFramePlanesChecked(dst.Y, dst.Cb, dst.Cr, dst.LumaStride, dst.ChromaStride, dst.ChromaFormatIDC, dst.PictureStructure, ref, cache, n, square, height, delta, list, dstY, dstCb, dstCr, srcXOffset, srcYOffset, qpelSize, chromaWidth, avg, scratch, trustedKernelArgs)
}

func h264MCDirPartFramePlanesMaybeValidated(dstYPlane []uint8, dstCbPlane []uint8, dstCrPlane []uint8, dstLumaStride int, dstChromaStride int, chromaFormatIDC int, dstPictureStructure int32, ref *h264PicturePlanes, cache *macroblockMotionCache, n int, square bool, height int, delta int, list int, dstY int, dstCb int, dstCr int, srcXOffset int, srcYOffset int, qpelSize int, chromaWidth int, avg bool, scratch *h264MotionCompScratch, dstValidated bool) error {
	if !dstValidated {
		return h264MCDirPartFramePlanes(dstYPlane, dstCbPlane, dstCrPlane, dstLumaStride, dstChromaStride, chromaFormatIDC, dstPictureStructure, ref, cache, n, square, height, delta, list, dstY, dstCb, dstCr, srcXOffset, srcYOffset, qpelSize, chromaWidth, avg, scratch)
	}
	if ref == nil {
		return ErrInvalidData
	}
	if err := ref.validate(); err != nil {
		return err
	}
	dst := h264PicturePlanes{
		Y: dstYPlane, Cb: dstCbPlane, Cr: dstCrPlane,
		LumaStride: dstLumaStride, ChromaStride: dstChromaStride,
		ChromaFormatIDC: chromaFormatIDC, PictureStructure: dstPictureStructure,
	}
	return h264MCDirPartFramePlanesCore(&dst, ref, cache, n, square, height, delta, list, dstY, dstCb, dstCr, srcXOffset, srcYOffset, qpelSize, chromaWidth, avg, scratch, true)
}

func h264MCDirPartFramePlanesChecked(dstYPlane []uint8, dstCbPlane []uint8, dstCrPlane []uint8, dstLumaStride int, dstChromaStride int, chromaFormatIDC int, dstPictureStructure int32, ref *h264PicturePlanes, cache *macroblockMotionCache, n int, square bool, height int, delta int, list int, dstY int, dstCb int, dstCr int, srcXOffset int, srcYOffset int, qpelSize int, chromaWidth int, avg bool, scratch *h264MotionCompScratch, trustedKernelArgs bool) error {
	if err := h264CheckMCDirPartFramePlanesArgs(dstLumaStride, dstChromaStride, chromaFormatIDC, ref, cache, n, height, delta, list); err != nil {
		return err
	}
	if dstLumaStride != ref.LumaStride || (chromaFormatIDC != 0 && dstChromaStride != ref.ChromaStride) {
		return ErrInvalidData
	}
	if ref.ChromaFormatIDC != chromaFormatIDC {
		return ErrInvalidData
	}
	dst := h264PicturePlanes{
		Y: dstYPlane, Cb: dstCbPlane, Cr: dstCrPlane,
		LumaStride: dstLumaStride, ChromaStride: dstChromaStride,
		ChromaFormatIDC: chromaFormatIDC, PictureStructure: dstPictureStructure,
	}
	return h264MCDirPartFramePlanesCore(&dst, ref, cache, n, square, height, delta, list, dstY, dstCb, dstCr, srcXOffset, srcYOffset, qpelSize, chromaWidth, avg, scratch, trustedKernelArgs)
}

func h264CheckMCDirPartFramePlanesArgs(dstLumaStride int, dstChromaStride int, chromaFormatIDC int, ref *h264PicturePlanes, cache *macroblockMotionCache, n int, height int, delta int, list int) error {
	if ref == nil || cache == nil || n < 0 || n >= 16 || list < 0 || list > 1 || dstLumaStride <= 0 || dstChromaStride < 0 || chromaFormatIDC < 0 || chromaFormatIDC > 3 || height <= 0 || delta < 0 {
		return ErrInvalidData
	}
	return nil
}

func h264MCDirPartFramePlanesCore(dst *h264PicturePlanes, ref *h264PicturePlanes, cache *macroblockMotionCache, n int, square bool, height int, delta int, list int, dstY int, dstCb int, dstCr int, srcXOffset int, srcYOffset int, qpelSize int, chromaWidth int, avg bool, scratch *h264MotionCompScratch, trustedKernelArgs bool) error {
	mv := cache.MV[list][h264Scan8[n]]
	mx := int(mv[0]) + srcXOffset*8
	my := int(mv[1]) + srcYOffset*8
	lumaXY := (mx & 3) + ((my & 3) << 2)
	fullMx := mx >> 2
	fullMy := my >> 2
	lumaWidth := qpelSize
	lumaHeight := qpelSize
	if !square {
		if delta < dst.LumaStride {
			lumaWidth += delta
		} else {
			lumaHeight += delta / dst.LumaStride
		}
	}
	extraWidth := 0
	extraHeight := 0
	if mx&7 != 0 {
		extraWidth -= 3
	}
	if my&7 != 0 {
		extraHeight -= 3
	}
	emu := false
	if fullMx < -extraWidth ||
		fullMy < -extraHeight ||
		fullMx+lumaWidth > ref.MBWidth*16+extraWidth ||
		fullMy+lumaHeight > ref.MBHeight*16+extraHeight {
		emu = true
	}

	srcY := fullMx + fullMy*ref.LumaStride
	srcYPlane := ref.Y
	srcLumaStride := ref.LumaStride
	if emu {
		edge, edgeStride, err := h264EdgeScratch(scratch, ref.LumaStride, lumaWidth+5, lumaHeight+5)
		if err != nil {
			return err
		}
		if err := h264EmulatedEdgeMC(edge, 0, edgeStride, ref.Y, ref.LumaStride, lumaWidth+5, lumaHeight+5, fullMx-2, fullMy-2, ref.MBWidth*16, ref.MBHeight*16); err != nil {
			return err
		}
		srcYPlane = edge
		srcLumaStride = edgeStride
		srcY = 2 + 2*edgeStride
	}
	if err := h264CallQpelMCStridesMaybeTrusted(dst.Y, dstY, dst.LumaStride, srcYPlane, srcY, srcLumaStride, qpelSize, lumaXY, avg, trustedKernelArgs); err != nil {
		return err
	}
	if !square {
		srcDelta := h264RemapDeltaForStride(delta, dst.LumaStride, srcLumaStride)
		if err := h264CallQpelMCStridesMaybeTrusted(dst.Y, dstY+delta, dst.LumaStride, srcYPlane, srcY+srcDelta, srcLumaStride, qpelSize, lumaXY, avg, trustedKernelArgs); err != nil {
			return err
		}
	}

	switch dst.ChromaFormatIDC {
	case 0:
		return nil
	case 3:
		return h264MCDirPartFrameChroma444Core(dst, ref, fullMx, fullMy, lumaXY, qpelSize, square, delta, dstCb, dstCr, avg, scratch, emu, trustedKernelArgs)
	case 1:
		if dst.PictureStructure == PictureFrame && ref.PictureStructure == PictureFrame {
			return h264MCDirPartFrameProgressive420ChromaCore(dst, ref, mx, my, height, dstCb, dstCr, chromaWidth, avg, scratch, trustedKernelArgs)
		}
		return h264MCDirPartFrameSubsampledChromaCore(dst, ref, mx, my, height, dstCb, dstCr, chromaWidth, avg, scratch, trustedKernelArgs)
	case 2:
		return h264MCDirPartFrameSubsampledChromaCore(dst, ref, mx, my, height, dstCb, dstCr, chromaWidth, avg, scratch, trustedKernelArgs)
	default:
		return ErrInvalidData
	}
}

func h264MCDirPartFrameProgressive420ChromaCore(dst *h264PicturePlanes, ref *h264PicturePlanes, mx int, my int, height int, dstCb int, dstCr int, chromaWidth int, avg bool, scratch *h264MotionCompScratch, trustedKernelArgs bool) error {
	chromaHeight := height >> 1
	chromaX := mx & 7
	chromaY := my & 7
	chromaBaseX := mx >> 3
	chromaBaseY := my >> 3
	picW := ref.MBWidth * 8
	picH := ref.MBHeight * 8
	extraChromaWidth := 0
	extraChromaHeight := 0
	if chromaX != 0 {
		extraChromaWidth = -1
	}
	if chromaY != 0 {
		extraChromaHeight = -1
	}
	if chromaBaseX < 0 || chromaBaseY < 0 ||
		chromaBaseX+chromaWidth > picW+extraChromaWidth ||
		chromaBaseY+chromaHeight > picH+extraChromaHeight {
		return h264MCDirPartFrameSubsampledChromaCore(dst, ref, mx, my, height, dstCb, dstCr, chromaWidth, avg, scratch, trustedKernelArgs)
	}
	srcC := chromaBaseX + chromaBaseY*ref.ChromaStride
	if srcC < 0 || dstCb < 0 || dstCr < 0 || srcC > len(ref.Cb) || srcC > len(ref.Cr) || dstCb > len(dst.Cb) || dstCr > len(dst.Cr) {
		return ErrInvalidData
	}
	if trustedKernelArgs {
		h264ChromaMCDualStridesKernel(dst.Cb[dstCb:], dst.Cr[dstCr:], ref.Cb[srcC:], ref.Cr[srcC:], dst.ChromaStride, ref.ChromaStride, int32(chromaHeight), int32(chromaX), int32(chromaY), int32(chromaWidth), avg)
		return nil
	}
	if err := h264ChromaMCStrides(dst.Cb[dstCb:], ref.Cb[srcC:], dst.ChromaStride, ref.ChromaStride, chromaHeight, chromaX, chromaY, chromaWidth, avg); err != nil {
		return err
	}
	return h264ChromaMCStrides(dst.Cr[dstCr:], ref.Cr[srcC:], dst.ChromaStride, ref.ChromaStride, chromaHeight, chromaX, chromaY, chromaWidth, avg)
}

func h264MCDirPartFrameChroma444Core(dst *h264PicturePlanes, ref *h264PicturePlanes, fullMx int, fullMy int, lumaXY int, qpelSize int, square bool, delta int, dstCb int, dstCr int, avg bool, scratch *h264MotionCompScratch, emu bool, trustedKernelArgs bool) error {
	srcC := fullMx + fullMy*ref.ChromaStride
	srcCbPlane := ref.Cb
	srcChromaStride := ref.ChromaStride
	if emu {
		edge, edgeStride, err := h264EdgeScratch(scratch, ref.ChromaStride, 16+5, 16+5)
		if err != nil {
			return err
		}
		if err := h264EmulatedEdgeMC(edge, 0, edgeStride, ref.Cb, ref.ChromaStride, 16+5, 16+5, fullMx-2, fullMy-2, ref.MBWidth*16, ref.MBHeight*16); err != nil {
			return err
		}
		srcCbPlane = edge
		srcChromaStride = edgeStride
		srcC = 2 + 2*edgeStride
	}
	if err := h264CallQpelMCStridesMaybeTrusted(dst.Cb, dstCb, dst.ChromaStride, srcCbPlane, srcC, srcChromaStride, qpelSize, lumaXY, avg, trustedKernelArgs); err != nil {
		return err
	}
	if !square {
		srcDelta := h264RemapDeltaForStride(delta, dst.ChromaStride, srcChromaStride)
		if err := h264CallQpelMCStridesMaybeTrusted(dst.Cb, dstCb+delta, dst.ChromaStride, srcCbPlane, srcC+srcDelta, srcChromaStride, qpelSize, lumaXY, avg, trustedKernelArgs); err != nil {
			return err
		}
	}
	srcCrPlane := ref.Cr
	srcChromaStride = ref.ChromaStride
	if emu {
		edge, edgeStride, err := h264EdgeScratch(scratch, ref.ChromaStride, 16+5, 16+5)
		if err != nil {
			return err
		}
		if err := h264EmulatedEdgeMC(edge, 0, edgeStride, ref.Cr, ref.ChromaStride, 16+5, 16+5, fullMx-2, fullMy-2, ref.MBWidth*16, ref.MBHeight*16); err != nil {
			return err
		}
		srcCrPlane = edge
		srcChromaStride = edgeStride
		srcC = 2 + 2*edgeStride
	}
	if err := h264CallQpelMCStridesMaybeTrusted(dst.Cr, dstCr, dst.ChromaStride, srcCrPlane, srcC, srcChromaStride, qpelSize, lumaXY, avg, trustedKernelArgs); err != nil {
		return err
	}
	if !square {
		srcDelta := h264RemapDeltaForStride(delta, dst.ChromaStride, srcChromaStride)
		return h264CallQpelMCStridesMaybeTrusted(dst.Cr, dstCr+delta, dst.ChromaStride, srcCrPlane, srcC+srcDelta, srcChromaStride, qpelSize, lumaXY, avg, trustedKernelArgs)
	}
	return nil
}

func h264MCDirPartFrameSubsampledChromaCore(dst *h264PicturePlanes, ref *h264PicturePlanes, mx int, my int, height int, dstCb int, dstCr int, chromaWidth int, avg bool, scratch *h264MotionCompScratch, trustedKernelArgs bool) error {
	yShift := 3
	chromaHeight := height
	chromaMy := my
	if dst.ChromaFormatIDC == 1 {
		if dstParity, ok := h264FieldParity(dst.PictureStructure); ok {
			if refParity, ok := h264FieldParity(ref.PictureStructure); ok {
				chromaMy += 2 * (dstParity - refParity)
			}
		}
	}
	chromaY := chromaMy & 7
	if dst.ChromaFormatIDC == 1 {
		chromaHeight >>= 1
	} else {
		yShift = 2
		chromaY = (chromaMy << 1) & 7
	}
	chromaX := mx & 7
	chromaBaseX := mx >> 3
	chromaBaseY := chromaMy >> yShift
	picW := ref.MBWidth * 8
	picH := ref.MBHeight * 16
	if dst.ChromaFormatIDC == 1 {
		picH >>= 1
	}
	extraChromaWidth := 0
	extraChromaHeight := 0
	if chromaX != 0 {
		extraChromaWidth = -1
	}
	if chromaY != 0 {
		extraChromaHeight = -1
	}
	chromaEmu := chromaBaseX < 0 || chromaBaseY < 0 ||
		chromaBaseX+chromaWidth > picW+extraChromaWidth ||
		chromaBaseY+chromaHeight > picH+extraChromaHeight
	srcC := chromaBaseX + chromaBaseY*ref.ChromaStride
	if !chromaEmu {
		if srcC < 0 || dstCb < 0 || dstCr < 0 || srcC > len(ref.Cb) || srcC > len(ref.Cr) || dstCb > len(dst.Cb) || dstCr > len(dst.Cr) {
			return ErrInvalidData
		}
		if trustedKernelArgs {
			h264ChromaMCDualStridesKernel(dst.Cb[dstCb:], dst.Cr[dstCr:], ref.Cb[srcC:], ref.Cr[srcC:], dst.ChromaStride, ref.ChromaStride, int32(chromaHeight), int32(chromaX), int32(chromaY), int32(chromaWidth), avg)
			return nil
		}
		if err := h264ChromaMCStridesMaybeTrusted(dst.Cb[dstCb:], ref.Cb[srcC:], dst.ChromaStride, ref.ChromaStride, chromaHeight, chromaX, chromaY, chromaWidth, avg, trustedKernelArgs); err != nil {
			return err
		}
		return h264ChromaMCStridesMaybeTrusted(dst.Cr[dstCr:], ref.Cr[srcC:], dst.ChromaStride, ref.ChromaStride, chromaHeight, chromaX, chromaY, chromaWidth, avg, trustedKernelArgs)
	}

	blockW := chromaWidth + 1
	blockH := chromaHeight + 1
	edge, edgeStride, err := h264EdgeScratch(scratch, ref.ChromaStride, blockW, blockH)
	if err != nil {
		return err
	}
	if err := h264EmulatedEdgeMC(edge, 0, edgeStride, ref.Cb, ref.ChromaStride, blockW, blockH, chromaBaseX, chromaBaseY, picW, picH); err != nil {
		return err
	}
	if dstCb < 0 || dstCb > len(dst.Cb) {
		return ErrInvalidData
	}
	if err := h264ChromaMCStridesMaybeTrusted(dst.Cb[dstCb:], edge, dst.ChromaStride, edgeStride, chromaHeight, chromaX, chromaY, chromaWidth, avg, trustedKernelArgs); err != nil {
		return err
	}

	edge, edgeStride, err = h264EdgeScratch(scratch, ref.ChromaStride, blockW, blockH)
	if err != nil {
		return err
	}
	if err := h264EmulatedEdgeMC(edge, 0, edgeStride, ref.Cr, ref.ChromaStride, blockW, blockH, chromaBaseX, chromaBaseY, picW, picH); err != nil {
		return err
	}
	if dstCr < 0 || dstCr > len(dst.Cr) {
		return ErrInvalidData
	}
	return h264ChromaMCStridesMaybeTrusted(dst.Cr[dstCr:], edge, dst.ChromaStride, edgeStride, chromaHeight, chromaX, chromaY, chromaWidth, avg, trustedKernelArgs)
}

func h264FieldParity(pictureStructure int32) (int, bool) {
	switch pictureStructure {
	case PictureTopField:
		return 0, true
	case PictureBottomField:
		return 1, true
	default:
		return 0, false
	}
}

func h264MBDestPartOffsets(dst *h264PicturePlanes, mbX int, mbY int, xOffset int, yOffset int) (int, int, int, error) {
	if dst == nil || mbX < 0 || mbY < 0 || xOffset < 0 || yOffset < 0 {
		return 0, 0, 0, ErrInvalidData
	}
	dstY := mbY*16*dst.LumaStride + mbX*16 + 2*xOffset + 2*yOffset*dst.LumaStride
	dstCb, dstCr := 0, 0
	switch dst.ChromaFormatIDC {
	case 0:
	case 1:
		dstCb = mbY*8*dst.ChromaStride + mbX*8 + xOffset + yOffset*dst.ChromaStride
		dstCr = dstCb
	case 2:
		dstCb = mbY*16*dst.ChromaStride + mbX*8 + xOffset + 2*yOffset*dst.ChromaStride
		dstCr = dstCb
	case 3:
		dstCb = mbY*16*dst.ChromaStride + mbX*16 + 2*xOffset + 2*yOffset*dst.ChromaStride
		dstCr = dstCb
	default:
		return 0, 0, 0, ErrInvalidData
	}
	return dstY, dstCb, dstCr, nil
}

func h264MCReference(refs *[2][]*h264PicturePlanes, cache *macroblockMotionCache, list int, n int) (*h264PicturePlanes, error) {
	if refs == nil || cache == nil || list < 0 || list > 1 || n < 0 || n >= 16 {
		return nil, ErrInvalidData
	}
	refIdx := cache.Ref[list][h264Scan8[n]]
	if refIdx < 0 || int(refIdx) >= len(refs[list]) || refs[list][refIdx] == nil {
		return nil, ErrInvalidData
	}
	return refs[list][refIdx], nil
}

func h264CallQpelMC(dst []uint8, dstOffset int, src []uint8, srcOffset int, stride int, size int, lumaXY int, avg bool) error {
	return h264CallQpelMCStrides(dst, dstOffset, stride, src, srcOffset, stride, size, lumaXY, avg)
}

func h264CallQpelMCStrides(dst []uint8, dstOffset int, dstStride int, src []uint8, srcOffset int, srcStride int, size int, lumaXY int, avg bool) error {
	mx := lumaXY & 3
	my := (lumaXY >> 2) & 3
	if avg {
		return h264QpelMCStrides(dst, dstOffset, dstStride, src, srcOffset, srcStride, size, mx, my, true)
	}
	return h264QpelMCStrides(dst, dstOffset, dstStride, src, srcOffset, srcStride, size, mx, my, false)
}

func h264CallQpelMCStridesMaybeTrusted(dst []uint8, dstOffset int, dstStride int, src []uint8, srcOffset int, srcStride int, size int, lumaXY int, avg bool, trusted bool) error {
	if !trusted {
		return h264CallQpelMCStrides(dst, dstOffset, dstStride, src, srcOffset, srcStride, size, lumaXY, avg)
	}
	h264QpelMCStridesKernel(dst, dstOffset, dstStride, src, srcOffset, srcStride, int32(size), int32(lumaXY&3), int32((lumaXY>>2)&3), avg)
	return nil
}

// h264ChromaMCStridesMaybeTrusted may skip the leaf wrapper checks only after
// the enclosing motion-comp path has validated both picture views and offsets.
func h264ChromaMCStridesMaybeTrusted(dst []uint8, src []uint8, dstStride int, srcStride int, height int, x int, y int, width int, avg bool, trusted bool) error {
	if !trusted {
		return h264ChromaMCStrides(dst, src, dstStride, srcStride, height, x, y, width, avg)
	}
	h264ChromaMCStridesKernel(dst, src, dstStride, srcStride, int32(height), int32(x), int32(y), int32(width), avg)
	return nil
}

func h264RemapDeltaForStride(delta int, dstStride int, srcStride int) int {
	if delta > 0 && dstStride > 0 && delta >= dstStride && delta%dstStride == 0 {
		return (delta / dstStride) * srcStride
	}
	return delta
}

func h264CheckMotionPlanePair(dst *h264PicturePlanes, ref *h264PicturePlanes) error {
	if err := dst.validate(); err != nil {
		return err
	}
	if err := ref.validate(); err != nil {
		return err
	}
	return h264CheckMotionPlanePairShape(dst, ref)
}

func h264CheckMotionPlanePairShape(dst *h264PicturePlanes, ref *h264PicturePlanes) error {
	if dst == nil || ref == nil {
		return ErrInvalidData
	}
	if dst.MBWidth != ref.MBWidth ||
		dst.MBHeight != ref.MBHeight ||
		dst.ChromaFormatIDC != ref.ChromaFormatIDC ||
		dst.LumaStride != ref.LumaStride ||
		dst.ChromaStride != ref.ChromaStride {
		return ErrInvalidData
	}
	if dst.ChromaFormatIDC == 3 && dst.ChromaStride != dst.LumaStride {
		return ErrUnsupported
	}
	return nil
}

func (p *h264PicturePlanes) validate() error {
	if p == nil {
		return ErrInvalidData
	}
	if p.trustedLayout {
		return nil
	}
	if p.MBWidth <= 0 || p.MBHeight <= 0 || p.LumaStride <= 0 || p.ChromaFormatIDC < 0 || p.ChromaFormatIDC > 3 {
		return ErrInvalidData
	}
	lumaWidth, err := checkedMulInt(p.MBWidth, 16)
	if err != nil {
		return ErrInvalidData
	}
	lumaHeight, err := checkedMulInt(p.MBHeight, 16)
	if err != nil {
		return ErrInvalidData
	}
	if p.LumaStride < lumaWidth || !h264PlaneHas(p.Y, p.LumaStride, lumaHeight, lumaWidth) {
		return ErrInvalidData
	}
	if p.ChromaFormatIDC == 0 {
		return nil
	}
	chromaWidth, chromaHeight, err := h264ChromaFrameSizeChecked(p.MBWidth, p.MBHeight, p.ChromaFormatIDC)
	if err != nil {
		return ErrInvalidData
	}
	if p.ChromaStride < chromaWidth || !h264PlaneHas(p.Cb, p.ChromaStride, chromaHeight, chromaWidth) || !h264PlaneHas(p.Cr, p.ChromaStride, chromaHeight, chromaWidth) {
		return ErrInvalidData
	}
	return nil
}

func h264PlaneSpanLength(stride int, height int, width int) (int, error) {
	return h264PlaneSpanEnd(0, stride, height, width)
}

func h264PlaneSpanEnd(offset int, stride int, height int, width int) (int, error) {
	if offset < 0 || stride <= 0 || height < 0 || width < 0 {
		return 0, ErrInvalidData
	}
	if height == 0 {
		return offset, nil
	}
	lastRow, err := checkedMulInt(height-1, stride)
	if err != nil {
		return 0, ErrInvalidData
	}
	end, err := checkedAddInt(offset, lastRow)
	if err != nil {
		return 0, ErrInvalidData
	}
	end, err = checkedAddInt(end, width)
	if err != nil {
		return 0, ErrInvalidData
	}
	return end, nil
}

func h264ChromaFrameSize(mbWidth int, mbHeight int, chromaFormatIDC int) (int, int) {
	width, height, err := h264ChromaFrameSizeChecked(mbWidth, mbHeight, chromaFormatIDC)
	if err != nil {
		return 0, 0
	}
	return width, height
}

func h264ChromaFrameSizeChecked(mbWidth int, mbHeight int, chromaFormatIDC int) (int, int, error) {
	switch chromaFormatIDC {
	case 1:
		width, err := checkedMulInt(mbWidth, 8)
		if err != nil {
			return 0, 0, ErrInvalidData
		}
		height, err := checkedMulInt(mbHeight, 8)
		if err != nil {
			return 0, 0, ErrInvalidData
		}
		return width, height, nil
	case 2:
		width, err := checkedMulInt(mbWidth, 8)
		if err != nil {
			return 0, 0, ErrInvalidData
		}
		height, err := checkedMulInt(mbHeight, 16)
		if err != nil {
			return 0, 0, ErrInvalidData
		}
		return width, height, nil
	case 3:
		width, err := checkedMulInt(mbWidth, 16)
		if err != nil {
			return 0, 0, ErrInvalidData
		}
		height, err := checkedMulInt(mbHeight, 16)
		if err != nil {
			return 0, 0, ErrInvalidData
		}
		return width, height, nil
	default:
		return 0, 0, ErrInvalidData
	}
}

func isSub8x8(mbType uint32) bool {
	return mbType&MBType16x16 != 0
}

func isSub8x4(mbType uint32) bool {
	return mbType&MBType16x8 != 0
}

func isSub4x8(mbType uint32) bool {
	return mbType&MBType8x16 != 0
}

func isSub4x4(mbType uint32) bool {
	return mbType&MBType8x8 != 0
}
