...
func TestValidateSimpleFrameSliceDecodeHighAllowsHigh10Chroma422FieldPictures(t *testing.T) {
	for _, tt := range []struct {
		...
		{name: "top/B-implicit-weight-mode1", pictureStructure: PictureTopField, sliceType: PictureTypeB, deblockMode: 1, implicitWeighted: true},
		{name: "top/B-implicit-weight-mode2", pictureStructure: PictureTopField, sliceType: PictureTypeB, deblockMode: 2, implicitWeighted: true},
		{name: "bottom/B-implicit-weight-mode1", pictureStructure: PictureBottomField, sliceType: PictureTypeB, deblockMode: 1, implicitWeighted: true},
		{name: "bottom/B-implicit-weight-mode2", pictureStructure: PictureBottomField, sliceType: PictureTypeB, deblockMode: 2, implicitWeighted: true},
	} {
...
func TestValidateSimpleFrameSliceDecodeHighAllowsHigh10ChromaFieldUnweightedBBoundary(t *testing.T) {
	for _, chroma := range []struct {
		...
		for _, picture := range []struct {
			...
			for _, cabac := range []bool{false, true} {
				...
				t.Run(fmt.Sprintf("%s/%s/%s/mode2", chroma.name, picture.name, entropy), func(t *testing.T) {
					m, dst, sh := highFrameSliceDecodeFixtureWithMBWidth(t, 10, chroma.idc, 2, true, PictureTypeB)
					sh.SPS.FrameMBSOnlyFlag = 0
					sh.SPS.MBAFF = 1
					sh.PictureStructure = picture.structure
					sh.DeblockingFilter = 2
					...
					if err := validateSimpleFrameSliceDecodeInputsHigh(m, dst, sh, 4); err != nil {
						t.Fatalf("high10 chroma unweighted field B boundary validation err = %v, want nil", err)
					}
				})
			}
		}
		...
		for _, cabac := range []bool{false, true} {
			for _, deblockMode := range []int32{0, 1, 2} {
				for _, weight := range weights {
					...
					t.Run(fmt.Sprintf("%s/%s/mode%d/%s", picture.name, entropy, deblockMode, weight.name), func(t *testing.T) {
						m, dst, sh := highFrameSliceDecodeFixtureWithMBWidth(t, 10, 2, 2, deblockMode != 0, PictureTypeB)
						sh.SPS.FrameMBSOnlyFlag = 0
						sh.SPS.MBAFF = 1
						sh.PictureStructure = picture.structure
...
func TestValidateSimpleFrameSliceDecodeHighAllowsHigh9ChromaWeightedPSliceBoundaryDeblock(t *testing.T) {
	weights := []struct {
		name  string
		table func(chromaFormatIDC int) PredWeightTable
	}{
		...
		for _, cabac := range []int32{0, 1} {
			for _, weight := range weights {
				t.Run(fmt.Sprintf("%s/cabac%d/%s", chromaFormatName(chromaFormatIDC), cabac, weight.name), func(t *testing.T) {
					m, dst, sh := highFrameSliceDecodeFixtureWithMBWidth(t, 9, chromaFormatIDC, 2, true, PictureTypeP)
					sh.DeblockingFilter = 2
					sh.RefCount = [2]uint32{1, 0}
					sh.PPS.CABAC = cabac
					sh.PPS.WeightedPred = 1
					...
					if err := validateSimpleFrameSliceDecodeInputsHigh(m, dst, sh, 4); err != nil {
						t.Fatalf("high9 chroma weighted P slice-boundary validation err = %v, want nil", err)
					}
				})
			}
		}
...
func TestValidateSimpleFrameSliceDecodeHighAllowsHigh9ChromaImplicitWeightedBSliceBoundaryDeblock(t *testing.T) {
	for _, chromaFormatIDC := range []int{2, 3} {
		for _, cabac := range []int32{0, 1} {
			for _, tt := range []struct {
				name            string
				...
				{name: "serialized"},
				{name: "initialized", useWeight: 2, useWeightChroma: 2},
			} {
				t.Run(fmt.Sprintf("%s/cabac%d/%s", chromaFormatName(chromaFormatIDC), cabac, tt.name), func(t *testing.T) {
					m, dst, sh := highFrameSliceDecodeFixtureWithMBWidth(t, 9, chromaFormatIDC, 2, true, PictureTypeB)
					sh.DeblockingFilter = 2
					sh.RefCount = [2]uint32{1, 1}
					sh.PPS.CABAC = cabac
					sh.PPS.WeightedBipredIDC = 2
					...
					if err := validateSimpleFrameSliceDecodeInputsHigh(m, dst, sh, 4); err != nil {
						t.Fatalf("high9 chroma implicit weighted B slice-boundary validation err = %v, want nil", err)
					}
				})
			}
		}
...
func TestValidateSimpleFrameSliceDecodeHighAllowsHigh9ChromaUnweightedBSliceBoundaryDeblock(t *testing.T) {
	for _, chromaFormatIDC := range []int{2, 3} {
		for _, cabac := range []int32{0, 1} {
			t.Run(fmt.Sprintf("%s/cabac%d", chromaFormatName(chromaFormatIDC), cabac), func(t *testing.T) {
				m, dst, sh := highFrameSliceDecodeFixtureWithMBWidth(t, 9, chromaFormatIDC, 2, true, PictureTypeB)
				sh.DeblockingFilter = 2
				sh.RefCount = [2]uint32{1, 1}
				sh.PPS.CABAC = cabac
				...
				if err := validateSimpleFrameSliceDecodeInputsHigh(m, dst, sh, 4); err != nil {
					t.Fatalf("high9 chroma unweighted B slice-boundary validation err = %v, want nil", err)
				}
			})
		}
	}
...
func TestValidateSimpleFrameSliceDecodeHighAllowsHigh9ChromaExplicitWeightedBDeblocking(t *testing.T) {
	for _, chromaFormatIDC := range []int{2, 3} {
		for _, deblockMode := range []int32{0, 1, 2} {
			for _, cabac := range []int32{0, 1} {
				for _, tt := range []struct {
					...
					t.Run(fmt.Sprintf("%s/deblock%d/cabac%d/%s", chromaFormatName(chromaFormatIDC), deblockMode, cabac, tt.name), func(t *testing.T) {
						m, dst, sh := highFrameSliceDecodeFixtureWithMBWidth(t, 9, chromaFormatIDC, 2, deblockMode != 0, PictureTypeB)
						sh.DeblockingFilter = deblockMode
						sh.RefCount = [2]uint32{1, 1}
						sh.PPS.CABAC = cabac
						sh.PPS.WeightedBipredIDC = 1
						...
						if err := validateSimpleFrameSliceDecodeInputsHigh(m, dst, sh, 4); err != nil {
							t.Fatalf("high9 chroma explicit weighted B deblock validation err = %v, want nil", err)
						}
					})
				}
			}
...
func TestValidateHighFrameSliceReconstructAllowsHigh12IntraResidualScope(t *testing.T) {
	...
	assertHigh1214Intra4x4ResidualEntropyScope(t, sh, "high12")
	...
	for _, tt := range []struct {
		...
		{name: "luma-dc", cbp: 0, cbpTable: 0x100},
		{name: "chroma-dc", cbp: 0x10, cbpTable: 0x50},
		{name: "partition-chroma-dc", cbp: 0x10, cbpTable: 0x1d0},
		{name: "chroma-dc-ac", cbp: 0x20, cbpTable: 0x60},
		{name: "luma-chroma", cbp: 0x2f, cbpTable: 0x16f},
		{name: "x264-luma-chroma", cbp: 0x2f, cbpTable: 0xef},
		{name: "luma-ac", cbp: 0x0f, cbpTable: 0x0f},
		{name: "luma-dc-ac", cbp: 0x0f, cbpTable: 0x10f},
	} {
		t.Run("intra16x16-reject-cabac-only-under-cavlc-"+tt.name, func(t *testing.T) {
			if err := validateHighFrameSliceMacroblockForReconstructWithSubMB(sh, MBTypeIntra16x16, nil, tt.cbp, tt.cbpTable); err != ErrUnsupported {
				t.Fatalf("high12 CAVLC Intra16x16 %s CABAC-only residual err = %v, want ErrUnsupported", tt.name, err)
			}
		})
	}
	sh.PPS.CABAC = 1
	for _, tt := range []struct {
		...
		t.Run("intra16x16-cabac-"+tt.name, func(t *testing.T) {
			if err := validateHighFrameSliceMacroblockForReconstructWithSubMB(sh, MBTypeIntra16x16, nil, tt.cbp, tt.cbpTable); err != nil {
				t.Fatalf("high12 CABAC Intra16x16 %s residual reconstruct validation err = %v, want nil", tt.name, err)
			}
		})
	}
...
func TestValidateHighFrameSliceReconstructAllowsHigh14IntraResidualScope(t *testing.T) {
	...
	assertHigh1214Intra4x4ResidualEntropyScope(t, sh, "high14")
	if err := validateHighFrameSliceMacroblockForReconstructWithSubMB(sh, MBTypeIntra16x16, nil, 0, 0); err != nil {
	...
	for _, tt := range []struct {
		...
		t.Run("intra16x16-reject-cabac-only-under-cavlc-"+tt.name, func(t *testing.T) {
			if err := validateHighFrameSliceMacroblockForReconstructWithSubMB(sh, MBTypeIntra16x16, nil, tt.cbp, tt.cbpTable); err != ErrUnsupported {
				t.Fatalf("high14 CAVLC Intra16x16 %s CABAC-only residual err = %v, want ErrUnsupported", tt.name, err)
			}
		})
	}
	for _, tt := range []struct {
...
func assertHigh1214Intra4x4ResidualEntropyScope(t *testing.T, sh *SliceHeader, label string) {
	t.Helper()
	if sh == nil || sh.PPS == nil {
		t.Fatalf("%s missing PPS for Intra4x4 residual entropy scope", label)
	}
	originalCABAC := sh.PPS.CABAC
	defer func() { sh.PPS.CABAC = originalCABAC }()
	...
	for _, tt := range []struct {
		...
		{name: "two-luma", cbp: 0x03, cbpTable: 0x03},
		{name: "two-luma-cavlc", cbp: 0x03, cbpTable: 0x7003},
		...
		t.Run(label+"-intra4x4-cavlc-"+tt.name, func(t *testing.T) {
			if err := validateHighFrameSliceMacroblockForReconstructWithSubMB(sh, MBTypeIntra4x4, nil, tt.cbp, tt.cbpTable); err != nil {
				t.Fatalf("%s CAVLC Intra4x4 %s residual reconstruct validation err = %v, want nil", label, tt.name, err)
			}
		})
	}
	for _, tt := range []struct {
		...
		{name: "partition-luma-chroma-13-cabac", cbp: 0x13, cbpTable: 0xd3},
		{name: "partition-luma-chroma-15-cabac", cbp: 0x15, cbpTable: 0xd5},
		{name: "partition-luma-chroma-17-cabac", cbp: 0x17, cbpTable: 0xd7},
		{name: "x264-luma-chroma-cabac", cbp: 0x2f, cbpTable: 0xef},
	} {
		t.Run(label+"-intra4x4-reject-cabac-only-under-cavlc-"+tt.name, func(t *testing.T) {
			if err := validateHighFrameSliceMacroblockForReconstructWithSubMB(sh, MBTypeIntra4x4, nil, tt.cbp, tt.cbpTable); err != ErrUnsupported {
				t.Fatalf("%s CAVLC Intra4x4 %s residual reconstruct validation err = %v, want ErrUnsupported", label, tt.name, err)
			}
		})
	}
	...
	for _, tt := range []struct {
		...
		t.Run(label+"-intra4x4-cabac-"+tt.name, func(t *testing.T) {
			if err := validateHighFrameSliceMacroblockForReconstructWithSubMB(sh, MBTypeIntra4x4, nil, tt.cbp, tt.cbpTable); err != nil {
				t.Fatalf("%s CABAC Intra4x4 %s residual reconstruct validation err = %v, want nil", label, tt.name, err)
			}
		})
	}
	for _, tt := range []struct {
		...
		t.Run(label+"-intra4x4-reject-cavlc-only-under-cabac-"+tt.name, func(t *testing.T) {
			if err := validateHighFrameSliceMacroblockForReconstructWithSubMB(sh, MBTypeIntra4x4, nil, tt.cbp, tt.cbpTable); err != ErrUnsupported {
				t.Fatalf("%s CABAC Intra4x4 %s residual reconstruct validation err = %v, want ErrUnsupported", label, tt.name, err)
			}
		})
	}
}
