...
| Green oracle rows | 214 |
| Known-red rows in `failures.jsonl` | 10 |
...
high-bit-depth field/inter streams, full error resilience, threading/SIMD and
...
- `FailureMatrix` runs the full 224-row manifest, requiring 214 green rows to
  match oracle output and 10 known-red rows to remain red until fixed.
