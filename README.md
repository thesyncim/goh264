...
- This safe point proves 16x16 and 32x32 no-skip non-direct B-frame CAVLC/CABAC parity over yuv420p output across Annex B, explicit AVC, and one-shot configured AVC, with FFmpeg rawvideo MD5 oracle coverage.
- Row-threaded/border-exchange deblocking for complex slice scheduling, high-bit-depth IntraPCM, high-bit-depth transform-bypass reconstruction, field/MBAFF motion-compensation remapping and reference-progress waits, B-direct/B-skip motion, implicit B-weight table construction, field/gap/error-resilience MMCO behavior, high-bit-depth/SIMD transform and DSP variants, streaming delayed-frame flush APIs, threading, and general `Decode` output are not yet implemented.
