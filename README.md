...
- This safe point proves 16x16 High 4:4:4 Predictive CAVLC/CABAC IDR/P frame parity with deblocking disabled across Annex B, explicit AVC, configured AVC, and configured sample-by-sample decode, with FFmpeg yuv444p rawvideo MD5 oracle coverage.
- Row-threaded/border-exchange deblocking for complex slice scheduling, high-bit-depth IntraPCM, transform-bypass reconstruction, 4:4:4 loop filtering, field/MBAFF motion-compensation remapping and reference-progress waits, B-direct/B-skip motion, B-list/direct reference handling, field/gap/error-resilience MMCO behavior, implicit-weight table construction, high-bit-depth/SIMD transform and DSP variants, full DPB/output ordering, threading, and general `Decode` output are not yet implemented.
