...
const (
	...
	high10DirectSubResidualCABACFile       = "high10_direct_sub_residual_cabac.h264"
	high10DirectSubResidualCABACAnnexBSize = 713
	high10DirectSubResidualCABACAnnexBMD5  = "b2a7308fa9760e1c28b623f55affbc41"
...
func TestHigh10DirectSubResidualCABACFixtureSyntax(t *testing.T) {
	data := readHigh10DirectSubResidualCABACFixture(t)
	if len(data) != high10DirectSubResidualCABACAnnexBSize {
		t.Fatalf("annex b size = %d, want %d", len(data), high10DirectSubResidualCABACAnnexBSize)
	}
	sum := md5.Sum(data)
	if got := hex.EncodeToString(sum[:]); got != high10DirectSubResidualCABACAnnexBMD5 {
		t.Fatalf("annex b md5 = %s, want %s", got, high10DirectSubResidualCABACAnnexBMD5)
	}
	assertHigh10DirectSubResidualCABACFixtureSyntax(t, data)
}
...
func TestDecodeAnnexBHigh10DirectSubResidualCABACFrames(t *testing.T) {
	data := readHigh10DirectSubResidualCABACFixture(t)
	assertHigh10DirectSubResidualCABACFixtureSyntax(t, data)
	frames, err := NewDecoder().DecodeAnnexBFrames(data)
	if err != nil {
...
func TestDecodeAVCHigh10DirectSubResidualCABACFrames(t *testing.T) {
	data := readHigh10DirectSubResidualCABACFixture(t)
	assertHigh10DirectSubResidualCABACFixtureSyntax(t, data)
	for _, nalLengthSize := range []int{2, 3, 4} {
		frames, err := NewDecoder().DecodeAVCFrames(annexBToAVC(t, data, nalLengthSize), nalLengthSize)
...
func TestDecodeConfiguredAVCHigh10DirectSubResidualCABACFramesAcrossSamplesFlush(t *testing.T) {
	data := readHigh10DirectSubResidualCABACFixture(t)
	assertHigh10DirectSubResidualCABACFixtureSyntax(t, data)
	for _, nalLengthSize := range []int{2, 3, 4} {
		config, samples := annexBToAVCConfigAndSamples(t, data, nalLengthSize)
...
func TestFFmpegRawVideoMD5OracleHigh10DirectSubResidualCABAC(t *testing.T) {
	if os.Getenv("GOH264_ORACLE") != "1" {
		t.Skip("set GOH264_ORACLE=1 to run native ffmpeg oracle")
	}
	if _, err := exec.LookPath("ffmpeg"); err != nil {
	...
	data := readHigh10DirectSubResidualCABACFixture(t)
	assertHigh10DirectSubResidualCABACFixtureSyntax(t, data)
	path := filepath.Join("testdata", "h264", high10DirectSubResidualCABACFile)
...
func readHigh10DirectSubResidualCABACFixture(t *testing.T) []byte {
	t.Helper()
	data, err := os.ReadFile(filepath.Join("testdata", "h264", high10DirectSubResidualCABACFile))
	if err != nil {
		t.Fatal(err)
	}
	return data
...
func assertHigh10DirectSubResidualCABACFixtureSyntax(t *testing.T, data []byte) {
	t.Helper()
	nals, err := h264.SplitAnnexB(data)
	if err != nil {
		t.Fatal(err)
	...
	for _, nal := range nals {
		...
		case h264.NALPPS:
			...
			if pps.CABAC != 1 || pps.Transform8x8Mode != 0 || pps.WeightedPred != 0 ||
				pps.WeightedBipredIDC != 0 || pps.RefCount[0] != 2 || pps.RefCount[1] != 1 {
				t.Fatalf("PPS cabac/8x8/weights/refs = %d/%d/%d/%d/%d/%d, want CABAC no-8x8 unweighted refs=2/1",
					pps.CABAC, pps.Transform8x8Mode, pps.WeightedPred, pps.WeightedBipredIDC, pps.RefCount[0], pps.RefCount[1])
			}
			ppsList[pps.PPSID] = pps
		case h264.NALIDRSlice, h264.NALSlice:
...
func assertHigh10DirectSubResidualCABACPayload(t *testing.T, raw []byte) {
	t.Helper()
	if len(raw) != 12 {
		t.Fatalf("B direct-sub residual CABAC NAL size = %d, want 12", len(raw))
	}
	if got, want := hex.EncodeToString(raw), "019e44e48bbe278af3a1bfc0"; got != want {
		t.Fatalf("B direct-sub residual CABAC payload = %s, want %s", got, want)
	}
}
