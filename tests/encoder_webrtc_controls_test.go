...
func TestEncoderRTPPayloadTypeZeroSelectsDynamicDefault(t *testing.T) {
	assertPayloadType := func(t *testing.T, out goh264.EncodedFrame, wantPayloadType uint8, wantSSRC uint32) {
		t.Helper()
		if out.OutputFormat != goh264.EncoderOutputRTP || len(out.RTPPackets) == 0 {
			t.Fatalf("RTP output = format %v packets %d, want emitted RTP packets", out.OutputFormat, len(out.RTPPackets))
		}
		for i, pkt := range out.RTPPackets {
			if err := pkt.Validate(); err != nil {
				t.Fatalf("packet[%d].Validate: %v", i, err)
			}
			if pkt.PayloadType != wantPayloadType {
				t.Fatalf("packet[%d] payload type = %d, want %d", i, pkt.PayloadType, wantPayloadType)
			}
			if got := pkt.Data[1] & 0x7f; got != wantPayloadType {
				t.Fatalf("packet[%d] header payload type = %d, want %d", i, got, wantPayloadType)
			}
			if pkt.SSRC != wantSSRC || binary.BigEndian.Uint32(pkt.Data[8:12]) != wantSSRC {
				t.Fatalf("packet[%d] SSRC = metadata %#x header %#x, want %#x",
					i, pkt.SSRC, binary.BigEndian.Uint32(pkt.Data[8:12]), wantSSRC)
			}
		}
	}
	...
	normalized, err := cfg.Normalize()
	if err != nil {
		t.Fatalf("Normalize payload type zero: %v", err)
	}
	if normalized.RTPPayloadType != 96 {
		t.Fatalf("Normalize payload type zero = %d, want dynamic default 96", normalized.RTPPayloadType)
	}
	enc, err := goh264.NewEncoder(cfg)
	if err != nil {
		t.Fatalf("NewEncoder payload type zero: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("Encode payload type default: %v", err)
	}
	assertPayloadType(t, out, 96, cfg.RTPSSRC)

	if err := enc.SetRTPMetadata(110, 0x22334455); err != nil {
		t.Fatalf("SetRTPMetadata nonzero: %v", err)
	}
	out, err = enc.Encode(frame)
	if err != nil {
		t.Fatalf("Encode payload type 110: %v", err)
	}
	assertPayloadType(t, out, 110, 0x22334455)

	if err := enc.SetRTPMetadata(0, 0x33445566); err != nil {
		t.Fatalf("SetRTPMetadata zero: %v", err)
	}
	...
	out, err = enc.Encode(frame)
	if err != nil {
		t.Fatalf("Encode payload type after SetRTPMetadata zero: %v", err)
	}
	assertPayloadType(t, out, 96, 0x33445566)

	payloadType := uint8(111)
	ssrc := uint32(0x44556677)
	if err := enc.Reconfigure(goh264.EncoderReconfigure{RTPPayloadType: &payloadType, RTPSSRC: &ssrc}); err != nil {
		t.Fatalf("Reconfigure nonzero payload type: %v", err)
	}
	payloadType = 0
	ssrc = 0x55667788
	if err := enc.Reconfigure(goh264.EncoderReconfigure{RTPPayloadType: &payloadType, RTPSSRC: &ssrc}); err != nil {
		t.Fatalf("Reconfigure zero payload type: %v", err)
	}
	if got := enc.Config(); got.RTPPayloadType != 96 || got.RTPSSRC != ssrc {
		t.Fatalf("Reconfigure zero config payload type/ssrc = %d/%#x, want 96/%#x",
			got.RTPPayloadType, got.RTPSSRC, ssrc)
	}
	out, err = enc.Encode(frame)
	if err != nil {
		t.Fatalf("Encode payload type after Reconfigure zero: %v", err)
	}
...
func TestEncoderDroppedFrameDoesNotPersistPerFrameForceIDR(t *testing.T) {
	budgets := []struct {
		name    string
		lower   goh264.EncoderReconfigure
		restore goh264.EncoderReconfigure
		...
		t.Run(format.name, func(t *testing.T) {
			for _, budget := range budgets {
				t.Run(budget.name, func(t *testing.T) {
					...
					forced := patternedI420EncoderFrame(16, 16)
					forced.PTS = 1234
					forced.Y[0] ^= 0x3d
					forced.ForceIDR = true
					dropped, err := enc.Encode(forced)
					if err != nil {
						t.Fatalf("Encode per-frame ForceIDR under %s budget: %v", budget.name, err)
					}
					if !dropped.Dropped || dropped.IDR || len(dropped.Data) != 0 || len(dropped.NALUnits) != 0 || len(dropped.RTPPackets) != 0 {
						t.Fatalf("budgeted per-frame ForceIDR output = %+v, want empty dropped metadata", dropped)
					}
					if enc.PendingIDR() {
						t.Fatal("budgeted per-frame ForceIDR drop queued pending IDR")
					}
					if callbackCalls != firstPacketCount {
						t.Fatalf("budgeted per-frame ForceIDR callbacks = %d, want still %d", callbackCalls, firstPacketCount)
					}
					...
					next := frame
					next.PTS = 2468
					second, err := enc.Encode(next)
					if err != nil {
						t.Fatalf("Encode after per-frame ForceIDR drop: %v", err)
					}
					if second.Dropped || second.IDR || second.KeyFrame || enc.PendingIDR() {
						t.Fatalf("post-drop output dropped=%v idr=%v key=%v pending=%v, want delivered P-skip",
							second.Dropped, second.IDR, second.KeyFrame, enc.PendingIDR())
					}
					assertEncoderNALTypes(t, second.NALUnits, []uint8{1})
					if format.fmt == goh264.EncoderOutputRTP {
					...
					if callbackCalls != firstPacketCount+len(second.RTPPackets) {
						t.Fatalf("post-drop callbacks = %d, want %d",
							callbackCalls, firstPacketCount+len(second.RTPPackets))
					}
					stream := annexBFromEncodedFrame(t, first, format.fmt)
					stream = append(stream, annexBFromEncodedFrame(t, second, format.fmt)...)
...
func TestEncoderDroppedIntervalIDRRetriesWithoutAdvancingCadence(t *testing.T) {
	budgets := []struct {
		name    string
		lower   goh264.EncoderReconfigure
		restore goh264.EncoderReconfigure
		...
		t.Run(format.name, func(t *testing.T) {
			for _, budget := range budgets {
				t.Run(budget.name, func(t *testing.T) {
					...
					second, err := enc.Encode(frame)
					if err != nil {
						t.Fatalf("Encode second P-skip: %v", err)
					}
					if second.Dropped || second.IDR || second.KeyFrame || enc.PendingIDR() {
						t.Fatalf("second output dropped=%v idr=%v key=%v pending=%v, want completed P-skip",
							second.Dropped, second.IDR, second.KeyFrame, enc.PendingIDR())
					}
					secondPacketCount := len(second.RTPPackets)
					if format.fmt == goh264.EncoderOutputRTP {
						if firstPacketCount == 0 || secondPacketCount == 0 || callbackCalls != firstPacketCount+secondPacketCount {
							t.Fatalf("pre-drop RTP packets/callbacks = %d/%d/%d, want nonzero matching count",
								firstPacketCount, secondPacketCount, callbackCalls)
						}
					} else if firstPacketCount != 0 || secondPacketCount != 0 || callbackCalls != 0 {
						t.Fatalf("non-RTP pre-drop packets/callbacks = %d/%d/%d, want none",
							firstPacketCount, secondPacketCount, callbackCalls)
					}
					...
					intervalIDR := patternedI420EncoderFrame(16, 16)
					intervalIDR.PTS = int64(2 * cfg.RTPTimestampIncrement)
					intervalIDR.Y[0] ^= 0x3d
					dropped, err := enc.Encode(intervalIDR)
					if err != nil {
						t.Fatalf("Encode interval IDR under %s budget: %v", budget.name, err)
					}
					if !dropped.Dropped || dropped.IDR || dropped.KeyFrame || len(dropped.Data) != 0 ||
						len(dropped.NALUnits) != 0 || len(dropped.RTPPackets) != 0 || enc.PendingIDR() {
						t.Fatalf("budgeted interval IDR output = %+v pending=%v, want empty dropped metadata without forced IDR",
							dropped, enc.PendingIDR())
					}
					if dropped.PTS != intervalIDR.PTS || dropped.DTS != intervalIDR.PTS || dropped.RTPTime != uint32(intervalIDR.PTS) {
						t.Fatalf("budgeted interval IDR metadata pts/dts/rtp = %d/%d/%d, want %d/%d/%d",
							dropped.PTS, dropped.DTS, dropped.RTPTime, intervalIDR.PTS, intervalIDR.PTS, uint32(intervalIDR.PTS))
					}
					if callbackCalls != firstPacketCount+secondPacketCount {
						t.Fatalf("budgeted interval IDR callbacks = %d, want still %d",
							callbackCalls, firstPacketCount+secondPacketCount)
					}

					if err := enc.Reconfigure(budget.restore); err != nil {
						t.Fatalf("restore %s: %v", budget.name, err)
					}
					recovered := intervalIDR
					recovered.PTS = int64(3 * cfg.RTPTimestampIncrement)
					third, err := enc.Encode(recovered)
					if err != nil {
						t.Fatalf("Encode after interval IDR drop: %v", err)
					}
					if third.Dropped || !third.IDR || !third.KeyFrame || enc.PendingIDR() {
						t.Fatalf("post-drop output dropped=%v idr=%v key=%v pending=%v, want delivered interval IDR",
							third.Dropped, third.IDR, third.KeyFrame, enc.PendingIDR())
					}
					assertEncoderNALTypes(t, third.NALUnits, []uint8{7, 8, 5})
					if format.fmt == goh264.EncoderOutputRTP {
						assertRTPPacketMetadata(t, third.RTPPackets, cfg.RTPPayloadType, cfg.RTPSSRC, uint16(firstPacketCount+secondPacketCount))
					} else if len(third.RTPPackets) != 0 {
						t.Fatalf("non-RTP third packets = %d, want none", len(third.RTPPackets))
					}
					if callbackCalls != firstPacketCount+secondPacketCount+len(third.RTPPackets) {
						t.Fatalf("post-drop callbacks = %d, want %d",
							callbackCalls, firstPacketCount+secondPacketCount+len(third.RTPPackets))
					}
					stream := annexBFromEncodedFrame(t, first, format.fmt)
					stream = append(stream, annexBFromEncodedFrame(t, second, format.fmt)...)
					stream = append(stream, annexBFromEncodedFrame(t, third, format.fmt)...)
					assertEncoderVCLFrameNums(t, stream, []uint8{5, 1, 5}, []uint32{0, 1, 2})
				})
			}
		})
	}
...
func TestEncoderSetGOPControlsAutomaticIDRCadence(t *testing.T) {
	...
	if err := enc.SetGOP(2, 2); err != nil {
		t.Fatalf("SetGOP: %v", err)
	}
	if got := enc.Config(); got.GOPSize != 2 || got.IDRInterval != 2 {
		t.Fatalf("GOP controls = %d/%d, want 2/2", got.GOPSize, got.IDRInterval)
	}
	if enc.PendingIDR() {
		t.Fatal("SetGOP queued unexpected IDR")
	}
	...
	second, err := enc.Encode(secondFrame)
	if err != nil {
		t.Fatalf("Encode post-SetGOP P-skip: %v", err)
	}
	if second.Dropped || second.IDR || second.KeyFrame || enc.PendingIDR() {
		t.Fatalf("post-SetGOP second frame dropped=%v idr=%v key=%v pending=%v, want delivered P-skip",
			second.Dropped, second.IDR, second.KeyFrame, enc.PendingIDR())
	}
	assertEncoderNALTypes(t, second.NALUnits, []uint8{1})
	...
	third, err := enc.Encode(thirdFrame)
	if err != nil {
		t.Fatalf("Encode automatic IDR after SetGOP: %v", err)
	}
	if third.Dropped || !third.IDR || enc.PendingIDR() {
		t.Fatalf("post-SetGOP third frame dropped=%v idr=%v pending=%v, want automatic IDR",
			third.Dropped, third.IDR, enc.PendingIDR())
	}
	assertEncoderNALTypes(t, third.NALUnits, []uint8{7, 8, 5})
...
func TestEncoderSetPresetPreservesLiveReference(t *testing.T) {
	cfg := goh264.DefaultEncoderConfig(16, 16)
	cfg.OutputFormat = goh264.EncoderOutputAnnexB
	cfg.DeblockMode = goh264.EncoderDeblockDisabled
	cfg.RTPMaxPayloadSize = 0
	...
	second, err := enc.Encode(secondFrame)
	if err != nil {
		t.Fatalf("Encode post-SetPreset P-skip: %v", err)
	}
	if second.Dropped || second.IDR || second.KeyFrame || enc.PendingIDR() {
		t.Fatalf("post-SetPreset frame dropped=%v idr=%v key=%v pending=%v, want delivered P-skip",
			second.Dropped, second.IDR, second.KeyFrame, enc.PendingIDR())
	}
	assertEncoderNALTypes(t, second.NALUnits, []uint8{1})
	...
	decodedSecond, err := dec.DecodeFrames(second.Data)
	if err != nil {
		t.Fatalf("Decode post-SetPreset P-skip: %v", err)
	}
	assertDecodedEncoderFrameBytes(t, decodedSecond, appendI420FrameBytes(nil, secondFrame))
...
func TestEncoderSPSPPSCadenceSettersControlLiveIDRHeaders(t *testing.T) {
	cfg := goh264.DefaultEncoderConfig(16, 16)
	cfg.OutputFormat = goh264.EncoderOutputAnnexB
	cfg.DeblockMode = goh264.EncoderDeblockDisabled
	cfg.RTPMaxPayloadSize = 0
	...
	if err := enc.SetSPSPPSMode(goh264.EncoderSPSPPSOutOfBand); err != nil {
		t.Fatalf("SetSPSPPSMode out-of-band: %v", err)
	}
	if enc.PendingIDR() {
		t.Fatal("SetSPSPPSMode queued unexpected IDR")
	}
	outOfBand := encodeForcedIDR("setter out-of-band", []uint8{5})

	if err := enc.SetSPSPPSMode(goh264.EncoderSPSPPSEveryIDR); err != nil {
		t.Fatalf("SetSPSPPSMode every-IDR: %v", err)
	}
	if err := enc.SetSPSPPSBeforeIDR(false); err != nil {
		t.Fatalf("SetSPSPPSBeforeIDR false: %v", err)
	}
	if enc.PendingIDR() {
		t.Fatal("SPS/PPS setters queued unexpected IDR")
	}
	everyIDR := encodeForcedIDR("setter every-IDR", []uint8{7, 8, 5})

	if err := enc.SetSPSPPSMode(goh264.EncoderSPSPPSInBandKeyframes); err != nil {
		t.Fatalf("SetSPSPPSMode in-band: %v", err)
	}
	if enc.PendingIDR() {
		t.Fatal("SetSPSPPSMode in-band queued unexpected IDR")
	}
	suppressed := encodeForcedIDR("setter in-band suppressed", []uint8{5})

	if err := enc.SetSPSPPSBeforeIDR(true); err != nil {
		t.Fatalf("SetSPSPPSBeforeIDR true: %v", err)
	}
	if got := enc.Config(); got.SPSPPSMode != goh264.EncoderSPSPPSInBandKeyframes || !got.SPSPPSBeforeIDR {
		t.Fatalf("restored SPS/PPS config = %+v, want in-band before IDR", got)
	}
	if enc.PendingIDR() {
		t.Fatal("SetSPSPPSBeforeIDR true queued unexpected IDR")
	}
	restored := encodeForcedIDR("setter in-band restored", []uint8{7, 8, 5})
...
func TestEncoderParameterSetsAppendHelpersReturnCallerOwnedBytes(t *testing.T) {
	...
	sps, err := headers.AppendSPS(append([]byte(nil), prefix...))
	if err != nil {
		t.Fatalf("AppendSPS: %v", err)
	}
	pps, err := headers.AppendPPS(append([]byte(nil), prefix...))
	if err != nil {
		t.Fatalf("AppendPPS: %v", err)
	}
	annexB, err := headers.AppendAnnexB(append([]byte(nil), prefix...))
	if err != nil {
		t.Fatalf("AppendAnnexB: %v", err)
	}
	avcc, err := headers.AppendAVCC(append([]byte(nil), prefix...))
	if err != nil {
		t.Fatalf("AppendAVCC: %v", err)
	}
...
func TestEncoderHelperClonesRejectOverflowedPublicStorage(t *testing.T) {
	overflowBytes := fakeDecoderRawBytesLen(maxIntForTest/2 + 1)
	for _, tt := range []struct {
		name string
		sets goh264.EncoderParameterSets
		...
		t.Run("parameter-sets-"+tt.name, func(t *testing.T) {
			...
			if !errors.Is(err, goh264.ErrInvalidData) || len(clone.SPS) != 0 ||
				...
				t.Fatalf("Clone overflow = %+v/%v, want empty ErrInvalidData", clone, err)
			}
		})
	}
...
func TestEncoderAppendHelpersRejectOverflowedPublicStorage(t *testing.T) {
	prefix := []byte{0xde, 0xad}
	overflowBytes := fakeDecoderRawBytesLen(maxIntForTest/2 + 1)
	...
	for _, tt := range []struct {
		name   string
		sets   goh264.EncoderParameterSets
		append func(goh264.EncoderParameterSets, []byte) ([]byte, error)
	}{
		{
			name: "sps",
			sets: goh264.EncoderParameterSets{SPS: overflowBytes},
			append: func(sets goh264.EncoderParameterSets, dst []byte) ([]byte, error) {
		...
		t.Run("parameter-sets-source-"+tt.name, func(t *testing.T) {
			got, err := tt.append(tt.sets, append([]byte(nil), prefix...))
			if !errors.Is(err, goh264.ErrInvalidData) || !bytes.Equal(got, prefix) {
				t.Fatalf("append overflow = len %d/%v, want preserved prefix ErrInvalidData", len(got), err)
			}
		})
		t.Run("parameter-sets-dst-"+tt.name, func(t *testing.T) {
			got, err := tt.append(validSets, largeDst)
			if !errors.Is(err, goh264.ErrInvalidData) || len(got) != len(largeDst) {
				t.Fatalf("append large dst = len %d/%v, want original dst ErrInvalidData", len(got), err)
			}
		})
	}
	...
	for _, tt := range []struct {
		name   string
		sei    goh264.EncoderSEI
		append func(goh264.EncoderSEI, []byte) ([]byte, error)
	}{
		{
			name: "nal",
			sei:  goh264.EncoderSEI{NAL: overflowBytes},
			append: func(sei goh264.EncoderSEI, dst []byte) ([]byte, error) {
		...
		t.Run("sei-source-"+tt.name, func(t *testing.T) {
			got, err := tt.append(tt.sei, append([]byte(nil), prefix...))
			if !errors.Is(err, goh264.ErrInvalidData) || !bytes.Equal(got, prefix) {
				t.Fatalf("append overflow = len %d/%v, want preserved prefix ErrInvalidData", len(got), err)
			}
		})
		t.Run("sei-dst-"+tt.name, func(t *testing.T) {
			got, err := tt.append(validSEI, largeDst)
			if !errors.Is(err, goh264.ErrInvalidData) || len(got) != len(largeDst) {
				t.Fatalf("append large dst = len %d/%v, want original dst ErrInvalidData", len(got), err)
			}
		})
...
func TestEncoderSEIAppendHelpersReturnCallerOwnedBytes(t *testing.T) {
	...
	nal, err := sei.AppendNAL(append([]byte(nil), prefix...))
	if err != nil {
		t.Fatalf("AppendNAL: %v", err)
	}
	annexB, err := sei.AppendAnnexB(append([]byte(nil), prefix...))
	if err != nil {
		t.Fatalf("AppendAnnexB: %v", err)
	}
	avc, err := sei.AppendAVC(append([]byte(nil), prefix...))
	if err != nil {
		t.Fatalf("AppendAVC: %v", err)
	}
	...
	tests := []struct {
		...
		},
		{
			...
			makeInput: func() overlapInput {
				...
				packet := encoderRTPPacketFromTestData(backing[2:17], backing[14:17])
				return overlapInput{
					dst: backing[:2],
					src: packet.Data,
					call: func(dst []byte) ([]byte, error) {
...
func TestEncoderMultiSliceResidualPBudgetRejectPreservesLiveReference(t *testing.T) {
	const (
		width      = 48
		height     = 32
		sliceCount = 3
		...
		t.Run(format.name, func(t *testing.T) {
			for _, budget := range []struct {
				...
				t.Run(budget.name, func(t *testing.T) {
					...
					reference := patternedI420EncoderFrame(width, height)
					reference.PTS = 0
					if _, err := probe.Encode(reference); err != nil {
						t.Fatalf("probe IDR: %v", err)
					}
					...
					assertEncoderNALTypes(t, probePSkip.NALUnits, repeatedEncoderNALTypes(uint8(h264.NALSlice), sliceCount))
					...
					residual := encoderP16x16SixMacroblockLumaDCResidualFrame(t, cfg, reference, int64(cfg.RTPTimestampIncrement))
					if err := budget.lower(enc); err != nil {
						t.Fatalf("lower %s: %v", budget.name, err)
					}
					...
					if !errors.Is(err, goh264.ErrInvalidData) {
						t.Fatalf("EncodeInto multi-slice residual P under %s error = %v, want ErrInvalidData", budget.name, err)
					}
					if rejected.Dropped || len(rejected.Data) != 0 || len(rejected.NALUnits) != 0 || len(rejected.RTPPackets) != 0 {
						t.Fatalf("rejected multi-slice residual P output = %+v, want empty output", rejected)
					}
					assertEncoderCallerBufferUnchanged(t, dst, backingBefore)
					if enc.PendingIDR() {
						t.Fatal("rejected multi-slice residual P queued unexpected IDR")
					}
					if callbackCalls != firstPacketCount {
						t.Fatalf("rejected multi-slice residual P callbacks = %d, want still %d", callbackCalls, firstPacketCount)
					}
					...
					recovered, err := enc.Encode(recoveryFrame)
					if err != nil {
						t.Fatalf("Encode after rejected multi-slice residual P: %v", err)
					}
					if recovered.Dropped || recovered.IDR || enc.PendingIDR() || len(recovered.Data) != pskipBytes {
						t.Fatalf("recovered frame dropped=%v idr=%v pending=%v data=%d, want P-skip size %d",
							recovered.Dropped, recovered.IDR, enc.PendingIDR(), len(recovered.Data), pskipBytes)
					}
					assertEncoderNALTypes(t, recovered.NALUnits, repeatedEncoderNALTypes(uint8(h264.NALSlice), sliceCount))

					stream := annexBFromEncodedFrame(t, first, cfg.OutputFormat)
					stream = append(stream, annexBFromEncodedFrame(t, recovered, cfg.OutputFormat)...)
					wantVCLTypes := repeatedEncoderNALTypes(uint8(h264.NALIDRSlice), sliceCount)
					wantVCLTypes = append(wantVCLTypes, repeatedEncoderNALTypes(uint8(h264.NALSlice), sliceCount)...)
					wantFrameNums := repeatedEncoderUint32s(0, sliceCount)
					wantFrameNums = append(wantFrameNums, repeatedEncoderUint32s(1, sliceCount)...)
					assertEncoderVCLFrameNums(t, stream, wantVCLTypes, wantFrameNums)
					wantFirstMBs := encoderExpectedSliceFirstMBs(width, height, sliceCount)
					wantStreamFirstMBs := append(append([]uint32(nil), wantFirstMBs...), wantFirstMBs...)
					assertEncoderVCLFirstMBs(t, stream, wantVCLTypes, wantStreamFirstMBs)

					decoded, err := goh264.NewDecoder().DecodeFrames(stream)
					if err != nil {
						t.Fatalf("Decode recovered multi-slice stream: %v", err)
					}
					if len(decoded) != 2 {
						t.Fatalf("decoded frames = %d, want 2", len(decoded))
					}
...
func TestEncoderReconfigureLimitsGroupUpdatesBudgetsAtomically(t *testing.T) {
	...
	singleLimitMaxFrameSize := 8192
	singleLimitSliceMaxBytes := 8192
	singleLimitMaxEncodeTimeUS := 20_000
	precedenceLimits := goh264.EncoderLimits{MaxFrameSize: 512, SliceMaxBytes: 256, MaxEncodeTimeUS: 5_000}
	if err := enc.Reconfigure(goh264.EncoderReconfigure{
		MaxFrameSize:         1024,
		SliceMaxBytes:        1024,
		MaxEncodeTimeUS:      15_000,
		MaxFrameSizeLimit:    &singleLimitMaxFrameSize,
		SliceMaxBytesLimit:   &singleLimitSliceMaxBytes,
		MaxEncodeTimeUSLimit: &singleLimitMaxEncodeTimeUS,
	...
	if err := enc.Reconfigure(goh264.EncoderReconfigure{
		...
		t.Fatalf("Reconfigure grouped limits with single-limit pointer fields: %v", err)
	}
	if got := enc.Config(); got.MaxFrameSize != 0 || got.SliceMaxBytes != 0 || got.MaxEncodeTimeUS != 0 {
...
func TestEncoderSetFrameDropModeTogglesDerivedBitrateBudget(t *testing.T) {
	...
	if enc.PendingIDR() {
		t.Fatal("SetFrameDropMode ToBitrate queued unexpected IDR")
	}
	dropped, err := enc.Encode(frame)
	if err != nil {
		t.Fatalf("Encode after SetFrameDropMode ToBitrate: %v", err)
	}
	if !dropped.Dropped || len(dropped.Data) != 0 || len(dropped.NALUnits) != 0 || len(dropped.RTPPackets) != 0 {
		t.Fatalf("ToBitrate low-budget output = %+v, want dropped metadata without output", dropped)
	}
	...
	if err := enc.SetFrameDropMode(goh264.EncoderFrameDropDisabled); err != nil {
		t.Fatalf("SetFrameDropMode disabled: %v", err)
	}
	if got := enc.Config(); got.FrameDrop != goh264.EncoderFrameDropDisabled {
		t.Fatalf("frame drop mode = %v, want Disabled", got.FrameDrop)
	}
	if enc.PendingIDR() {
		t.Fatal("SetFrameDropMode disabled queued unexpected IDR")
	}
	recovered, err := enc.Encode(frame)
	if err != nil {
		t.Fatalf("Encode after SetFrameDropMode disabled: %v", err)
	}
	if recovered.Dropped || recovered.IDR || len(recovered.Data) != pskipBytes {
		t.Fatalf("disabled-drop recovered output dropped=%v idr=%v data=%d, want transmitted P-skip size %d",
			recovered.Dropped, recovered.IDR, len(recovered.Data), pskipBytes)
...
func TestEncoderSetRateControlTogglesDerivedBitrateBudget(t *testing.T) {
	probeFrame := patternedI420EncoderFrame(16, 16)
	probeCfg := goh264.DefaultEncoderConfig(16, 16)
	probeCfg.DeblockMode = goh264.EncoderDeblockDisabled
	probeCfg.FrameDrop = goh264.EncoderFrameDropDisabled
	...
	if err := enc.SetRateControl(goh264.EncoderRateControlCBR); err != nil {
		t.Fatalf("SetRateControl CBR: %v", err)
	}
	if got := enc.Config().RateControl; got != goh264.EncoderRateControlCBR {
		t.Fatalf("rate control = %v, want CBR", got)
	}
	if enc.PendingIDR() {
		t.Fatal("SetRateControl CBR queued unexpected IDR")
	}
	cbrFrame := frame
	cbrFrame.PTS = frame.PTS + int64(cfg.RTPTimestampIncrement)
	cbrDropped, err := enc.Encode(cbrFrame)
	...
	if err := enc.SetRateControl(goh264.EncoderRateControlConstantQP); err != nil {
		t.Fatalf("SetRateControl ConstantQP: %v", err)
	}
	if got := enc.Config().RateControl; got != goh264.EncoderRateControlConstantQP {
		t.Fatalf("rate control = %v, want ConstantQP", got)
	}
	if enc.PendingIDR() {
		t.Fatal("SetRateControl ConstantQP queued unexpected IDR")
	}
	recoveredFrame := frame
	recoveredFrame.PTS = cbrFrame.PTS + int64(cfg.RTPTimestampIncrement)
	recovered, err := enc.Encode(recoveredFrame)
...
func TestEncodedFrameRTPDataRejectsInvalidIndexesAndMetadata(t *testing.T) {
	...
	stapBPacketData := rtpPacketWithPayload(25, 0, 1, 0x67)
	mtap16PacketData := rtpPacketWithPayload(26, 0, 1, 0x67)
	mtap24PacketData := rtpPacketWithPayload(27, 0, 1, 0x67)
	fuBPacketData := rtpPacketWithPayload(29, 0x85, 0x99)
	...
	for _, tt := range []struct {
		...
		{name: "payload zero nal type", frame: goh264.EncodedFrame{OutputFormat: goh264.EncoderOutputRTP, RTPPackets: []goh264.EncoderRTPPacket{encoderRTPPacketFromTestData(zeroTypePayloadPacketData, zeroTypePayloadPacketData[12:])}}},
		{name: "stap-b payload", frame: goh264.EncodedFrame{OutputFormat: goh264.EncoderOutputRTP, RTPPackets: []goh264.EncoderRTPPacket{encoderRTPPacketFromTestData(stapBPacketData, stapBPacketData[12:])}}},
		{name: "mtap16 payload", frame: goh264.EncodedFrame{OutputFormat: goh264.EncoderOutputRTP, RTPPackets: []goh264.EncoderRTPPacket{encoderRTPPacketFromTestData(mtap16PacketData, mtap16PacketData[12:])}}},
		{name: "mtap24 payload", frame: goh264.EncodedFrame{OutputFormat: goh264.EncoderOutputRTP, RTPPackets: []goh264.EncoderRTPPacket{encoderRTPPacketFromTestData(mtap24PacketData, mtap24PacketData[12:])}}},
		{name: "fu-b payload", frame: goh264.EncodedFrame{OutputFormat: goh264.EncoderOutputRTP, RTPPackets: []goh264.EncoderRTPPacket{encoderRTPPacketFromTestData(fuBPacketData, fuBPacketData[12:])}}},
		{name: "truncated stapa length", frame: goh264.EncodedFrame{OutputFormat: goh264.EncoderOutputRTP, RTPPackets: []goh264.EncoderRTPPacket{encoderRTPPacketFromTestData(truncatedSTAPAPacketData, truncatedSTAPAPacketData[12:])}}},
		{name: "zero stapa nal size", frame: goh264.EncodedFrame{OutputFormat: goh264.EncoderOutputRTP, RTPPackets: []goh264.EncoderRTPPacket{encoderRTPPacketFromTestData(zeroSTAPAPacketData, zeroSTAPAPacketData[12:])}}},
		{name: "oversized stapa nal", frame: goh264.EncodedFrame{OutputFormat: goh264.EncoderOutputRTP, RTPPackets: []goh264.EncoderRTPPacket{encoderRTPPacketFromTestData(oversizedSTAPAPacketData, oversizedSTAPAPacketData[12:])}}},
		{name: "stapa inner forbidden zero bit", frame: goh264.EncodedFrame{OutputFormat: goh264.EncoderOutputRTP, RTPPackets: []goh264.EncoderRTPPacket{encoderRTPPacketFromTestData(forbiddenInnerSTAPAPacketData, forbiddenInnerSTAPAPacketData[12:])}}},
		{name: "stapa inner zero nal type", frame: goh264.EncodedFrame{OutputFormat: goh264.EncoderOutputRTP, RTPPackets: []goh264.EncoderRTPPacket{encoderRTPPacketFromTestData(zeroInnerSTAPAPacketData, zeroInnerSTAPAPacketData[12:])}}},
		{name: "stapa inner stapa", frame: goh264.EncodedFrame{OutputFormat: goh264.EncoderOutputRTP, RTPPackets: []goh264.EncoderRTPPacket{encoderRTPPacketFromTestData(nestedSTAPAInSTAPAPacketData, nestedSTAPAInSTAPAPacketData[12:])}}},
		{name: "stapa inner fua", frame: goh264.EncodedFrame{OutputFormat: goh264.EncoderOutputRTP, RTPPackets: []goh264.EncoderRTPPacket{encoderRTPPacketFromTestData(nestedFUAInSTAPAPacketData, nestedFUAInSTAPAPacketData[12:])}}},
		{name: "short fua", frame: goh264.EncodedFrame{OutputFormat: goh264.EncoderOutputRTP, RTPPackets: []goh264.EncoderRTPPacket{encoderRTPPacketFromTestData(shortFUAPacketData, shortFUAPacketData[12:])}}},
...
func TestEncoderRTPPacketDataHelpersReturnClippedCallerOwnedBytes(t *testing.T) {
	...
	paddingHeaderPacketData := append([]byte(nil), packetData...)
	paddingHeaderPacketData[0] = 0xa0
	extensionHeaderPacketData := append([]byte(nil), packetData...)
	extensionHeaderPacketData[0] = 0x90
	csrcHeaderPacketData := append([]byte(nil), packetData...)
	csrcHeaderPacketData[0] = 0x81
	...
	sequenceMismatchPacket.SequenceNumber++
	timestampMismatchPacket := encoderRTPPacketFromTestData(packetData, packetData[12:])
	timestampMismatchPacket.Timestamp++
	ssrcMismatchPacket := encoderRTPPacketFromTestData(packetData, packetData[12:])
	ssrcMismatchPacket.SSRC++
	...
	for _, tt := range []struct {
		...
		{name: "payload zero nal type", packet: encoderRTPPacketFromTestData(zeroPayloadPacketData, zeroPayloadPacketData[12:])},
		{name: "stap-b payload", packet: encoderRTPPacketFromTestData(stapBPacketData, stapBPacketData[12:])},
		{name: "mtap16 payload", packet: encoderRTPPacketFromTestData(mtap16PacketData, mtap16PacketData[12:])},
		{name: "mtap24 payload", packet: encoderRTPPacketFromTestData(mtap24PacketData, mtap24PacketData[12:])},
		{name: "fu-b payload", packet: encoderRTPPacketFromTestData(fuBPacketData, fuBPacketData[12:])},
		{name: "malformed stapa", packet: encoderRTPPacketFromTestData(malformedSTAPAPacketData, malformedSTAPAPacketData[12:])},
...
func TestEncodedFrameCloneRejectsInvalidMetadata(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{
			name:  "empty non-dropped frame",
			frame: goh264.EncodedFrame{},
		},
		{
			name: "rtp packets without access-unit metadata",
			frame: goh264.EncodedFrame{
				OutputFormat: goh264.EncoderOutputRTP,
				RTPPackets:   []goh264.EncoderRTPPacket{validRTPPacket},
			},
		},
		{
		...
		{
			name: "rtp packet metadata mismatch",
			frame: goh264.EncodedFrame{
				OutputFormat: goh264.EncoderOutputRTP,
				Data:         []byte{0, 0, 0, 1, 0x65},
				NALUnits:     []goh264.EncoderNALUnit{{Type: 5, Offset: 4, Size: 1, KeyFrame: true}},
		...
		},
		{
			...
			frame: goh264.EncodedFrame{
				...
				Data:         []byte{0, 0, 0, 1, 0x65},
				NALUnits:     []goh264.EncoderNALUnit{{Type: 5, Offset: 4, Size: 1, KeyFrame: true}},
				RTPPackets:   []goh264.EncoderRTPPacket{encoderRTPPacketFromTestData(validPacket[:11], validPacket[12:])},
			},
		},
		{
			...
			frame: goh264.EncodedFrame{
				...
				Data:         []byte{0, 0, 0, 1, 0x65},
				NALUnits:     []goh264.EncoderNALUnit{{Type: 5, Offset: 4, Size: 1, KeyFrame: true}},
				RTPPackets:   []goh264.EncoderRTPPacket{encoderRTPPacketFromTestData(validPacket, []byte{0x65})},
			},
		},
		{
...
func TestEncodedFrameOutputHelpersRejectOverflowedPublicStorage(t *testing.T) {
	...
	validPacket := encoderRTPPacketFromTestData(validPacketData, validPacketData[12:])
	prefix := []byte{1, 2}
	...
	overflowPayloadPacket := encoderRTPPacketFromTestData(validPacketData, overflowBytes)
	...
	overflowRTPPayloadFrame := goh264.EncodedFrame{
		...
		Data:         validAccessUnit,
		NALUnits:     validNALUnits,
		RTPPackets:   []goh264.EncoderRTPPacket{encoderRTPPacketFromTestData(validPacketData, overflowBytes)},
	}
...
func TestEncoderRealtimeWebRTCControlSurfaceCoversRoadmap(t *testing.T) {
	...
	for _, field := range []string{
		...
		"RTPMaxPayloadSize", "Limits", "MaxFrameSize", "MaxFrameSizeLimit",
		"MaxEncodeTimeUS", "MaxEncodeTimeUSLimit", "SliceCount", "SliceMaxBytes", "SliceMaxBytesLimit",
...
func encoderRTPPacketFromTestData(data []byte, payload []byte) goh264.EncoderRTPPacket {
	packet := goh264.EncoderRTPPacket{
		Data:    data,
		Payload: payload,
	}
...
func encoderP16x16ChromaOnlyResidualFrame(t *testing.T, cfg goh264.EncoderConfig, reference goh264.EncoderFrame, pts int64) goh264.EncoderFrame {
	t.Helper()
	return encoderP16x16SyntheticResidualFrame(t, cfg, reference, pts, h264.EncoderI420P16x16ResidualConfig{
		LumaCoefficients: [][]h264.EncoderResidualCoefficient{
			{},
...
func encoderP16x16SyntheticResidualFrame(t *testing.T, cfg goh264.EncoderConfig, reference goh264.EncoderFrame, pts int64, residualCfg h264.EncoderI420P16x16ResidualConfig) goh264.EncoderFrame {
	...
	residualCfg.Width = cfg.Width
	residualCfg.Height = cfg.Height
	residualCfg.FrameNum = 1
	residualCfg.InitialQP = cfg.InitialQP
	residualCfg.NextQP = cfg.InitialQP
	residualCfg.DisableDeblockingFilterIDC = 1
	residualCfg.NALLengthSize = 4
	residual, err := h264.BuildEncoderI420P16x16ResidualSlice(residualCfg)
	if err != nil {
