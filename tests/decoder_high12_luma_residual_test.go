...
const (
	high12LumaResidualBitstreamMD5 = "b969b426ba94bfc420dc4d405b09b0de"
	high12LumaResidualFrameMD5     = "0b19ad36fba5819e7195269b985d221a"
	high12LumaResidualRawVideoMD5  = "0b19ad36fba5819e7195269b985d221a"
	high12LumaResidualRawSize      = 2304
)

func TestHigh12LumaResidualFixtureSyntax(t *testing.T) {
	assertHigh12LumaResidualFixtureSyntax(t, readHigh12LumaResidualFixture(t))
}

func TestDecodeAnnexBHigh12LumaResidualFrame(t *testing.T) {
	data := readHigh12LumaResidualFixture(t)
	assertHigh12LumaResidualFixtureSyntax(t, data)

	frames, err := NewDecoder().DecodeAnnexBFrames(data)
	if err != nil {
		t.Fatalf("decode High12 luma residual Annex B: %v", err)
	}
	assertHigh12LumaResidualFrames(t, frames)
}

func TestDecodeAVCHigh12LumaResidualFrame(t *testing.T) {
	data := readHigh12LumaResidualFixture(t)
	assertHigh12LumaResidualFixtureSyntax(t, data)
...
func TestFFmpegRawVideoMD5OracleHigh12LumaResidual(t *testing.T) {
	...
	path := high12LumaResidualFixturePath(t)
	...
	line := []byte(fmt.Sprintf("0, %10d, %10d,        1, %8d, %s", 0, 0, high12LumaResidualRawSize, high12LumaResidualFrameMD5))
	...
	if len(raw) != high12LumaResidualRawSize {
		t.Fatalf("rawvideo size = %d, want %d", len(raw), high12LumaResidualRawSize)
	}
	sum := md5.Sum(raw)
	if got := hex.EncodeToString(sum[:]); got != high12LumaResidualRawVideoMD5 {
		t.Fatalf("rawvideo md5 = %s, want %s", got, high12LumaResidualRawVideoMD5)
	}
}

func readHigh12LumaResidualFixture(t *testing.T) []byte {
	t.Helper()
	data, err := os.ReadFile(high12LumaResidualFixturePath(t))
	if err != nil {
		t.Fatal(err)
	}
	sum := md5.Sum(data)
	if got := hex.EncodeToString(sum[:]); got != high12LumaResidualBitstreamMD5 {
		t.Fatalf("High12 luma residual bitstream md5 = %s, want %s", got, high12LumaResidualBitstreamMD5)
	}
	return data
}

func high12LumaResidualFixturePath(t *testing.T) string {
	t.Helper()
	return filepath.Join("testdata", "h264", "high12_luma_residual_cavlc_i.h264")
}

func assertHigh12LumaResidualFrames(t *testing.T, frames []*Frame) {
	t.Helper()
	if len(frames) != 1 {
	...
	if frame.Width != 48 || frame.Height != 16 ||
		...
		t.Fatalf("frame format = %dx%d chroma %d depth %d/%d, want 48x16 yuv420p12le",
	...
	if size, err := frame.RawYUVSize(); err != nil || size != high12LumaResidualRawSize {
		t.Fatalf("RawYUVSize = %d/%v, want %d/nil", size, err, high12LumaResidualRawSize)
	}
	...
	sum := md5.Sum(raw)
	if got := hex.EncodeToString(sum[:]); got != high12LumaResidualFrameMD5 {
		t.Fatalf("frame raw md5 = %s, want %s", got, high12LumaResidualFrameMD5)
	}
...
func assertHigh12LumaResidualFixtureSyntax(t *testing.T, data []byte) {
	t.Helper()
	...
	for _, nal := range nals {
		...
		case h264.NALSPS:
			...
			if sps.ProfileIDC != 244 || sps.Width != 48 || sps.Height != 16 ||
				...
				t.Fatalf("SPS profile/format = %d %dx%d chroma %d depth %d/%d, want High 4:4:4 Predictive-compatible 48x16 yuv420p12le",
		...
		case h264.NALIDRSlice, h264.NALSlice:
			...
			assertHigh12LumaResidualMacroblockSyntax(t, nal, sh.SPS, sh.PPS)
...
func assertHigh12LumaResidualMacroblockSyntax(t *testing.T, nal h264.NALUnit, sps *h264.SPS, pps *h264.PPS) {
	t.Helper()
	...
	assertHigh12LumaResidualBits(t, &br, "mb0 luma-DC", "00100110101")
	assertHigh12LumaResidualBits(t, &br, "mb1 luma-AC", "0000100001110101"+strings.Repeat("1", 15))
	assertHigh12LumaResidualBits(t, &br, "mb2 luma-DC/AC", "0000100001101010101"+strings.Repeat("1", 15))
	assertHigh12LumaResidualRBSPTrailingBits(t, &br)
}

func assertHigh12LumaResidualBits(t *testing.T, br *high10ResidualCAVLCBitReader, label string, bits string) {
	t.Helper()
...
func assertHigh12LumaResidualRBSPTrailingBits(t *testing.T, br *high10ResidualCAVLCBitReader) {
	t.Helper()
