...
proves accepted bitrate, frame-rate, RTP payload-size, max-frame-size,
slice-byte, encode-time, and deblock setter or reconfigure updates, including
SPS/PPS cadence and recovery-point toggles, preserve a queued IDR until the
next successful IDR encode,
...
slice-byte, and encode-time budgets including zero-valued budget disablement,
