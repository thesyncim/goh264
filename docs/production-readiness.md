...
for 1/2/3/4-byte AVC length sizes. Empty configuration-record packets also
flush delayed output before an incompatible avcC can reset picture state, then
store the new configuration for following configured-AVC packets. Zero-frame
...
Package-level `InspectAVCC` provides stateless avcC metadata inspection without
touching decoder state. Damaged configuration rejection leaves an existing
...
`ConfigureAVCC` calls and auto-detected `DecodeFrames`
...
prediction, bounded luma-DC, chroma-only, and combined luma/chroma residual-P
admission, changed-frame P IntraPCM recovery pictures, Annex B/AVC output, and
...
destination unchanged on invalid or overflowed-destination appends, isolate
...
packet data, malformed RTP payload rejection for admitted single-NAL, STAP-A,
and FU-A packet/payload byte helpers including STAP-B, MTAP, FU-B, nested
STAP-A units, and FU-A fragments whose reconstructed NAL type is another
packetization unit, payload byte parity against the access-unit NAL list,
frame-level FU-A start/continuation/end sequence validation, packet storage
isolated from `EncodedFrame.Data`
including caller-backed `EncodeInto` output buffers, unchanged caller append
destinations on invalid or overflowed-destination access-unit, NAL, RTP packet,
and RTP payload appends, isolated overlapping source/destination helper appends,
rejected malformed dropped-result storage in `EncodedFrame.Clone`, shared
...
residual dispatcher round-trips trailing-ones, single-level, single-level-plus-trailing-ones,
non-trailing, and multi-level-plus-trailing-one residual blocks through `decodeCAVLCResidual`
across luma and chroma-DC CAVLC table contexts, Annex B/AVC NAL packaging, AVC decoder configuration records,
