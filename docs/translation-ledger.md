...
| `libavcodec/h264_slice.c` `h264_slice_header_parse` through ref-count handoff | `internal/h264/slice.go` `ParseSliceHeader` | translated | unit, `ffprobe` fixture exercised through header parse | Stops before reference-list reordering, prediction weights, CABAC init, and macroblock decode. IDR `idr_pic_id >= 65536` follows FFmpeg's warning-only behavior by leaving the stored value unset. |
...
| `libavcodec/h264_slice.c` remaining slice decode | none | stubbed | not yet proved | Pending after reference-list reordering, prediction weights, and entropy slices. |
