...
size/latency budget, output/cadence, RTP packetization, and RTP payload
metadata controls now have explicit setter helpers; `EncoderReconfigure`
remains the grouped update path for bundled multi-field changes and explicit
ForceIDR requests.
...
admits the frame, explicit max-frame-size, slice-byte, and encode-time setters
allow zero-valued runtime budget disablement, and ConstantQP mode is proved to
bypass that derived budget before and after runtime switches through CBR.
`FrameDropLate` now uses
...
across Annex B, configured AVC, RTP reassembly, and RTP packetization-mode 0
single-NAL output. Mixed per-macroblock deblock fallback is also proved across
those same output surfaces.
...
5. In progress: add RTP packetization and WebRTC control handling with
   ...
   across transmitted and dropped frames, queued forced-IDR preservation across
   frame-size and slice-size drops, plus stale-credit reset after runtime max-bitrate/VBV lowering,
   ...
   drop/recovery, while explicit max-frame-size, slice-byte, and encode-time
   setters allow zero-valued runtime budget disablement. `FrameDropLate`
   bypasses that derived bitrate budget when the
...
- Allocation, race, and realtime budget gates are checked into the normal
  release evidence. The admitted local vet, contract, writer, allocation, and
