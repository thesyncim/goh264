...
- `internal/h264/simple_decode.go` now represents decoded frames with either
  ...
  weighted P16x16 P-slice with deblocking disabled.
...
- `internal/h264/reconstruct_high.go` has a separate `h264PicturePlanesHigh`
  ...
  weighted P16x16 pictures; partitioned P, B motion, and high
...
- `internal/h264/reconstruct_high.go` now consumes high motion internally for
  ...
  deblock-disabled P16x16 L0 residual slices and explicit weighted P16x16
  slices for CAVLC and CABAC. Partitioned P, P intra macroblocks, B slices,
  high deblocking, other chroma/depth combinations, and MBAFF remain guarded.
...
| Entropy-to-state | CAVLC/CABAC frame-MB handoff, residuals, motion caches, direct motion, and high IntraPCM payload sizing exist; high CAVLC/CABAC slice loops now carry deblock-disabled I pictures plus proved High 10 P-skip/P16x16 no-residual, exact P16x16 L0 residual, and explicit weighted P16x16 subsets through reconstruction. | Add high-specific regression cases where QP exceeds 51 only with matching public proof, then continue to partitioned P and B with separate oracles. |
...
| Public output | Public `Frame` exposes `Y16/Cb16/Cr16`, `RawPixelFormat`, `RawYUVSize`, `BytesPerSample`, `AppendRawYUV16`, and `AppendRawYUVBytesLE`; `AppendRawYUV` remains 8-bit-only; High 10 deblock-disabled I output, no-residual P-skip/P16x16 output, exact P16x16 L0 residual output, and explicit weighted P16x16 output are proved against FFmpeg rawvideo MD5s. | Keep partitioned P, B, high deblocking, GBR, MBAFF, and unproved chroma/depth combinations guarded. |
| Oracle fixtures | Kernel oracles cover high primitives; public frame-MD5 fixtures cover 8-bit High-profile streams, true High 10 CAVLC/CABAC deblock-disabled IDR/I fixtures, true High 10 IDR/P P-skip/P16x16 no-residual fixtures, true High 10 exact P16x16 L0 residual fixtures, and true High 10 explicit weighted P16x16 fixtures across Annex B/AVC/configured surfaces. | Build the next oracle targets for partitioned P and B pictures without widening the current guard. |
...
public output, while partitioned P, P intra macroblocks, B, high deblocking, and
...
- Keep high ref-list construction wired through the simple slice path for the
  proved High 10 P-skip/P16x16 no-residual, exact P16x16 residual, and explicit
  weighted P16x16 lanes.
...
- Keep the narrowed public high-bit-depth guards for residual P outside this
  exact lane, partitioned P, B, high deblock, and unproved chroma/depth modes
  until each path passes a framemd5/rawvideo oracle.
...
4. **Wire High 10 P-Slice No-Residual**
   ...
   - Guard after the weighted safe point: partitioned P, P intra macroblocks, B,
     high deblocking, and unproved depth/chroma combinations remain unsupported.
...
5. **Wire High 10 P-Slice P16x16 Residual**
   ...
   - Guard: 16x8/8x16/8x8 partitions, P intra macroblocks, B, high deblocking,
     other chroma/depth combinations, and MBAFF remain
     unsupported.
