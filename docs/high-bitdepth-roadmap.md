...
- `internal/h264/simple_decode.go` now represents decoded frames with either
  ...
  partitioned high-deblock lanes.
...
- The current B-picture safe point is deliberately narrow: exact High 10 4:2:0
  ...
  deblock-enabled streams. Mixed
  direct/explicit B8x8, residual-bearing direct-sub, broader partitioned
  implicit weighted B, and implicit-weighted or residual direct-sub high B
  deblocking remain separate oracle lanes.
- `internal/h264/reconstruct_high.go` has a separate `h264PicturePlanesHigh`
  ...
  the neutral and implicit-weighted partitioned high-deblocking lanes, the High
  ...
  implicit-weighted or residual direct-sub high B deblocking,
...
- `internal/h264/reconstruct_high.go` now consumes high motion internally for
  ...
  broader partitioned implicit weighted B, implicit-weighted or residual direct-sub high B deblocking,
...
B, implicit-weighted or residual direct-sub high B deblocking, and unproved
...
- PPS weighted bipred remains neutral (`weighted_bipred_idc == 0`) for
  direct-sub and explicit-partition fixtures; implicit weighted B is limited to
  proved B16x16 and partitioned B16x8/B8x16/B8x8 streams.
...
5. Later: 12-bit 4:2:2 deblocking, 14-bit 4:4:4 deblocking, CABAC/chroma/B-slice
   slice-boundary, and implicit-weighted or residual direct-sub high B
   deblocking.
...
14. High-depth deblocking enabled for 4:2:0, 4:2:2, and 4:4:4.
15. Annex B, explicit AVC/NALFF, configured AVC, sample-by-sample configured
...
10. **Wire High Loop Filter**
   ...
   - Remaining tests: CABAC/chroma/B-slice public high slice-boundary proof,
     implicit-weighted or residual direct-sub high B deblocking, broader 12-bit/all 14-bit, and
