...
0 allocs/op. The amd64 8-bit luma intra vertical leaf now uses SSE2 word
arithmetic with the same `uint8_t`/`ptrdiff_t`/`int` ABI shape; focused medians
are about 32.5 ns/op default versus about 70.6 ns/op in `purego`, all
0 allocs/op. The luma intra horizontal wrapper remains scalar until the wider
8x16 transpose path is ported and measured faster. The clean amd64
`caba3-sva-b` real-vector gate at
`aa74c63a9daaba0a8577171ab26c7c6323f830a8` passes raw-MD5 and the
zero-allocation Go gate with default partial asm at 155.1 ms/sample
(12.300 ns/raw-byte), versus 178.9 ms/sample (14.277 ns/raw-byte) in
`purego`, 60.9 ms/sample (4.876 ns/raw-byte) for FFmpeg pure-C, and
47.2 ms/sample (3.786 ns/raw-byte) for FFmpeg native C+asm.
...
1. Continue 8-bit deblock coverage with amd64 luma intra horizontal, luma
   MBAFF, and any remaining perf-positive chroma shapes now that normal luma
   vertical/horizontal, luma intra vertical, and the enabled chroma leaves have
   FFmpeg-shaped amd64 assembly.
2. Extend high-bit deblock coverage after the remaining 8-bit hot leaves are
   verified.
