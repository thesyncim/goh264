...
and motion-comp orchestration rather than a single missing leaf. On `caba3`
(`cabac`, `temporal-direct`, `deblock`), a same-host 50-iteration, 9-repeat A/B
moves clean `5ed6a43` from 540.783 ms/sample to 371.887 ms/sample, a 31.2%
end-to-end reduction, with exact raw-video MD5 parity and the zero-steady-state
allocation gate enabled. The fair paired run measures FFmpeg pure-C at 217.770
ms/sample and native C+asm at 157.041 ms/sample, so the current Go lane is still
1.71x slower than pure C and 2.37x slower than native FFmpeg; the goal is not
yet complete. A PGO build from the same workload measures 357.266 ms/sample,
another 3.9% below the normal Go build, but still 1.64x behind pure C. The
refreshed profile puts qpel near 5%, down from about 21% before the width-16/8
NEON port; CABAC, loop filtering, and call/layout overhead are now the main
frontier.
