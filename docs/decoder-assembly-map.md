...
| 2 | Chroma MC 8-bit width 8/4/2 put and avg | partial | partial | Width 8 `x=0,y=0` put/avg leaves are implemented in `internal/h264/chroma_mc_amd64.s` and `internal/h264/chroma_mc_arm64.s`. Remaining leaves: width 8 fractional x/y, width 4/2/1 put/avg. Kernel seam uses `[]uint8`, native signed pointer-width strides, and 32-bit C-int selectors. Verify against `TestH264Chroma*`, `chroma_oracle_test.go`, and real-vector `caba3`/`canl4`. |
