...
luma and chroma deblock ports, clean commit
`d0272d01abf3f7e500443fa76c30732719a25a80` moves `caba3` from
122.6 ms/sample before the deblock work to 104.8 ms/sample on the current
default partial-asm build, a 14.5% end-to-end reduction. The same paired run is
still about 1.82x slower than FFmpeg pure-C and 2.40x slower than FFmpeg native
C+asm, while `-tags=purego` is about 2.50x slower than FFmpeg pure-C and 3.17x
slower than FFmpeg native C+asm in its paired run. Current default partial
assembly is about 1.35x faster than `purego` on this vector.
...
0 allocs/op. The amd64 8-bit luma intra vertical and horizontal leaves now use
SSE2 word arithmetic with the same `uint8_t`/`ptrdiff_t`/`int` ABI shape; the
horizontal leaf transposes a 16x8 stack scratch tile, calls the shared vertical
core, and transposes back. Focused medians are about 32.4/47.8 ns/op default
versus about 69.6/69.0 ns/op in `purego`, all 0 allocs/op. The clean amd64
`caba3-sva-b` real-vector gate at
`d0272d01abf3f7e500443fa76c30732719a25a80` passes raw-MD5 and the
zero-allocation Go gate with default partial asm at 154.9 ms/sample
(12.267 ns/raw-byte), versus 177.1 ms/sample (14.152 ns/raw-byte) in
...
1. Continue 8-bit deblock coverage with luma MBAFF and any remaining
   perf-positive chroma shapes now that normal luma vertical/horizontal, luma
   intra vertical/horizontal, and the enabled chroma leaves have FFmpeg-shaped
   amd64 assembly.
