...
| --- | --- | --- | --- | --- |
| 1 | Qpel 8-bit size 16/8 all positions | partial | partial | `mc00` put/avg leaves for sizes 16/8/4/2 and `mc20` put/avg leaves for sizes 16/8 are implemented in `internal/h264/qpel_mc_amd64.s` and `internal/h264/qpel_mc_arm64.s`. Remaining leaves: `mc10`, `mc30`, vertical, diagonal, and HV qpel positions, plus smaller `mc20` sizes. Kernel seam in `internal/h264/qpel.go` uses `[]uint8`, native signed pointer-width strides/offsets, and 32-bit C-int selectors. Verify against `TestH264Qpel*`, `qpel_oracle_test.go`, and real-vector `caba3`/`canl4`. |
