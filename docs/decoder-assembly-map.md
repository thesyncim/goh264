...
| Luma qpel motion compensation | `.upstream/ffmpeg-n8.0.1/libavcodec/x86/h264_qpel_8bit.asm`, `h264_qpel_10bit.asm`, `h264_qpel.c`, plus `x86/fpel.asm` and `x86/qpel.asm` support | `.upstream/ffmpeg-n8.0.1/libavcodec/aarch64/h264qpel_neon.S`, `h264qpel_init_aarch64.c`, plus mc00 helpers in `aarch64/hpeldsp_neon.S` | `internal/h264/qpel.go`, `internal/h264/qpel_dispatch_asm.go`, `internal/h264/qpel_dispatch_purego.go`, `internal/h264/qpel_mc_amd64.s`, `internal/h264/qpel_mc_arm64.s`, call sites in `internal/h264/motion_comp.go` and `internal/h264/motion_comp_high.go` | partial | Largest motion-comp family and a direct profile hit. `h264QpelMCStridesKernel` keeps `uint8_t`/`ptrdiff_t` selector width parity with separate source/destination strides in the Go dispatch wrapper. Enabled leaves: 8-bit horizontal row `mc00`/`mc10`/`mc20`/`mc30`, vertical row `mc01`/`mc02`/`mc03`, center HV `mc22`, and H+V diagonal positions `mc11`/`mc31`/`mc13`/`mc33`, sizes 16/8, put/avg; plus `mc00` sizes 4/2, on amd64 and arm64. Remaining leaves: HV-blend diagonal positions `mc21`/`mc12`/`mc32`/`mc23` and smaller fractional sizes. High-bit-depth remains scalar until a byte-stride `uint8_t*` ABI is added for the 10-bit kernels. |
...
| --- | --- | --- | --- | --- |
| 1 | Qpel 8-bit size 16/8 all positions | partial | partial | Horizontal row leaves `mc00`/`mc10`/`mc20`/`mc30`, vertical row leaves `mc01`/`mc02`/`mc03`, center HV `mc22`, and H+V diagonal positions `mc11`/`mc31`/`mc13`/`mc33` put/avg for sizes 16/8 are implemented in `internal/h264/qpel_mc_amd64.s` and `internal/h264/qpel_mc_arm64.s`; `mc00` also covers sizes 4/2. Remaining leaves: HV-blend diagonal positions `mc21`/`mc12`/`mc32`/`mc23` plus smaller fractional sizes. Kernel seam in `internal/h264/qpel.go` uses `[]uint8`, native signed pointer-width strides/offsets, and 32-bit C-int selectors. Verify against `TestH264Qpel*`, `qpel_oracle_test.go`, and real-vector `caba3`/`canl4`. |
