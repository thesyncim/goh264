...
geometric elapsed ratio is 1.5665 with a two-sided 95% confidence interval of
[1.5480, 1.5852], and the 12-worker ratio is 1.5698 with a confidence interval
of [1.5320, 1.6085]. Both lanes produce raw-video MD5
...
| --- | --- | --- | --- | --- | --- |
| Chroma motion compensation | `.upstream/ffmpeg-n8.0.1/libavcodec/x86/h264_chromamc.asm`, `h264_chromamc_10bit.asm`, `h264chroma_init.c` | `.upstream/ffmpeg-n8.0.1/libavcodec/aarch64/h264cmc_neon.S`, `h264chroma_init_aarch64.c` | `internal/h264/chroma.go`, `internal/h264/chroma_dispatch_asm.go`, `internal/h264/chroma_dispatch_purego.go`, `internal/h264/chroma_dispatch_high_asm.go`, `internal/h264/chroma_dispatch_high.go`, `internal/h264/chroma_dual_arm64.s`, `internal/h264/chroma_mc_amd64.s`, `internal/h264/chroma_mc_arm64.s`, `internal/h264/chroma_fractional_amd64.s`, `internal/h264/chroma_fractional_arm64.s`, `internal/h264/chroma_fractional_neon_arm64.s`, `internal/h264/chroma_high_amd64.s`, `internal/h264/chroma_high_arm64.s`, call sites in `internal/h264/motion_comp.go` and `internal/h264/motion_comp_high.go` | verified | Widths 1/2/4/8, put/avg, 8-bit and high-bit-depth. The arm64 8-bit fractional width-8/4/2 leaves use NEON with independent source/destination strides; width-4 moved from roughly 26-31 ns/op to 12-14 ns/op and width-2 from 18-21 ns/op to 12-14 ns/op locally, all 0 allocs/op. The progressive 4:2:0 motion path now isolates its smaller stack frame and batches identical Cb/Cr fractional work through one coefficient setup and assembly boundary; the focused width-8 dual-plane median moves from about 13.2 ns to 9.7 ns, with exact scalar differential coverage and zero allocations. Width-1 and high-bit-depth arm64 remain scalar assembly. The high-bit-depth seam keeps the FFmpeg `uint8_t*` byte-pointer ABI and native byte strides. |
| Luma qpel motion compensation | `.upstream/ffmpeg-n8.0.1/libavcodec/x86/h264_qpel_8bit.asm`, `h264_qpel_10bit.asm`, `h264_qpel.c`, plus `x86/fpel.asm` and `x86/qpel.asm` support | `.upstream/ffmpeg-n8.0.1/libavcodec/aarch64/h264qpel_neon.S`, `h264qpel_init_aarch64.c`, plus mc00 helpers in `aarch64/hpeldsp_neon.S` | `internal/h264/qpel.go`, `internal/h264/qpel_dispatch_asm.go`, `internal/h264/qpel_dispatch_purego.go`, `internal/h264/qpel_dispatch_high_asm.go`, `internal/h264/qpel_dispatch_high.go`, `internal/h264/qpel_mc_amd64.s`, `internal/h264/qpel_mc_arm64.s`, `internal/h264/qpel_hvblend_amd64.s`, `internal/h264/qpel_hvblend_arm64.s`, `internal/h264/qpel_h_neon_arm64.s`, `internal/h264/qpel_v_neon_arm64.s`, `internal/h264/qpel_hv_neon_words_arm64.s`, `internal/h264/qpel_4_neon_arm64.s`, `internal/h264/qpel_high_amd64.s`, `internal/h264/qpel_high_arm64.s`, call sites in `internal/h264/motion_comp.go` and `internal/h264/motion_comp_high.go` | partial | All positions are assembly-backed and oracle-verified. On arm64, width-16/8 and every width-4 fractional shape now use NEON. The width-4 kernels read only the validated 9-byte horizontal or 9x9 two-dimensional footprint and retain independent source/destination strides. Focused axis modes move from roughly 32-36 ns/op to 16-18 ns/op, odd/odd modes from roughly 45-52 ns/op to about 18 ns/op, center-22 from roughly 72-75 ns/op to about 19 ns/op, and the representative HV-blend 21 mode from roughly 86-88 ns/op to 22-23 ns/op, all at 0 allocs/op. Width-2 fractional and high-bit-depth entrypoints remain scalar assembly. The connected `caba3-sva-b` gate passes exact raw-MD5 and zero-allocation checks while reducing the default Go gap against native libavcodec C+assembly in both one-worker and 12-worker lanes. |
...
| Intra prediction | `.upstream/ffmpeg-n8.0.1/libavcodec/x86/h264_intrapred.asm`, `h264_intrapred_10bit.asm`, `h264_intrapred_init.c` | `.upstream/ffmpeg-n8.0.1/libavcodec/aarch64/h264pred_neon.S`, `h264pred_init.c` | `internal/h264/pred.go`, `internal/h264/pred4x4_simple_arm64.s`, `internal/h264/pred4x4_angular_arm64.s`, `internal/h264/pred_high.go`, dispatch by mode in `internal/h264/intra_prediction.go` and reconstruction files | partial | Arm64 4x4 vertical, horizontal, DC, left-DC, top-DC, DC-128, down-right, and horizontal-up modes use checked assembly seams. The angular kernels match their scalar definitions over 4,096 randomized layouts and pass the pinned upstream oracle; down-right moves from roughly 9.0 ns/op in `purego` to about 7.3 ns/op and horizontal-up from roughly 7.9 ns/op to about 6.7 ns/op, all with zero allocations. The other angular 4x4 modes plus 8x8, 16x16, chroma, and high-bit modes remain scalar. |
| CABAC helper optimizations | `.upstream/ffmpeg-n8.0.1/libavcodec/x86/cabac.h`, `h264_cabac.c` | `.upstream/ffmpeg-n8.0.1/libavcodec/aarch64/cabac.h` | `internal/h264/cabac.go`, `internal/h264/cabac_mb.go`, `internal/h264/cabac_mvd_arm64.s`, `internal/h264/cabac_intra_pred_mode_arm64.s`, `internal/h264/cabac_residual.go`, `internal/h264/cabac_residual_significance_arm64.s`, `internal/h264/cabac_frame.go` | partial | The concrete slice decoder hand-inlines get/bypass/sign arithmetic, specializes residual and intra-prediction bins, and masks the pinned 0..9 normalization shift so arm64 emits native register shifts without redundant guards. Arm64 now batches the common MVD prefix/sign path, the previous-mode flag plus three explicit intra-4x4 mode bins, and fixed 16/15-coefficient significance scans while keeping scalar implementations as differential oracles. Their LPS selection and state-table indexing stay in 32-bit registers, matching CABAC's exact wrap semantics without constructing a full-width mask. Focused zero-allocation lanes improve by about 21% for MVD, 15% for the intra-4x4 mode batch, and 14% for fixed significance versus their scalar oracles. The batched paths retain stateful differential coverage, including 4,096 consecutive intra-mode decodes. Native libavcodec still fuses more CABAC and macroblock work, so further experiments must batch enough useful work to amortize every Go-to-assembly boundary. |
...
2. Finish arm64 width-2 qpel only when a representative vector proves a
   connected win; width-16/8/4 fractional shapes are already NEON.
...
| 2 | Luma qpel arm64 NEON | verified scalar asm | partial | Width-16/8 and all width-4 fractional shapes use NEON. The width-4 port passes exact scalar and upstream-oracle checks, cuts focused modes by roughly 2-4x, and improves both connected native-C+assembly comparison lanes. Width-2 fractional leaves remain scalar assembly and need connected evidence before porting. |
...
| 6 | Intra prediction 4x4/8x8/16x16/chroma | partial | partial | Arm64 simple 4x4 modes are oracle-covered and faster in focused and connected lanes. Angular 4x4 and larger/high-bit families remain. |
| functional | Qpel 8-bit sizes 16/8/4/2 all positions | verified | verified mixed SIMD/scalar asm | All qpel positions are assembly-backed. Arm64 width-16/8/4 fractional shapes are NEON; width-2 fractional shapes remain scalar assembly. Kernel seam retains separate native-width strides and 32-bit selectors. |
