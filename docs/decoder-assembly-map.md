...
| Intra prediction | `.upstream/ffmpeg-n8.0.1/libavcodec/x86/h264_intrapred.asm`, `h264_intrapred_10bit.asm`, `h264_intrapred_init.c` | `.upstream/ffmpeg-n8.0.1/libavcodec/aarch64/h264pred_neon.S`, `h264pred_init.c` | `internal/h264/pred.go`, `internal/h264/pred4x4_simple_arm64.s`, `internal/h264/pred_high.go`, dispatch by mode in `internal/h264/intra_prediction.go` and reconstruction files | partial | Arm64 4x4 vertical, horizontal, DC, left-DC, top-DC, and DC-128 modes use one checked assembly seam. The pinned upstream oracle passes; focused modes improve from roughly 8-14 ns/op in `purego` to about 5.5-6.1 ns/op, all with zero allocations. Angular 4x4, 8x8, 16x16, chroma, and high-bit modes remain scalar. |
