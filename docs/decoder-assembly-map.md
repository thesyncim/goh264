...
The amd64 8-bit normal luma vertical leaf now uses FFmpeg-shaped SSE2 math
instead of the previous scalar-equivalent assembly; on the local virtualized
amd64 lane the focused row moves from about 74 ns/op in `purego` to about
19 ns/op in default builds, with 0 allocs/op. The matching horizontal amd64
luma leaf is still scalar-equivalent and should use FFmpeg's transpose wrapper
around the vertical filter as the next amd64 deblock slice.
