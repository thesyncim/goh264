...
| Loop/deblock filter | `.upstream/ffmpeg-n8.0.1/libavcodec/x86/h264_deblock.asm`, `h264_deblock_10bit.asm`, `h264dsp_init.c` | `.upstream/ffmpeg-n8.0.1/libavcodec/aarch64/h264dsp_neon.S`, `h264dsp_init_aarch64.c` | wrappers in `internal/h264/dsp.go`, seam in `internal/h264/deblock_dispatch.go`, integration in `internal/h264/loop_filter.go` | seam | Visible after the edge-emulation scalar fix on deblocked `caba3`. The Go dispatch seam narrows FFmpeg C `int` thresholds to checked `int32` values, keeps strides as native signed `int`, and preserves scalar behavior for MBAFF, chroma 4:2:2, intra/non-intra, and high-bit-depth variants. |
...
| 6 | Loop filter 8-bit/high luma/chroma | seam | seam | `internal/h264/deblock_dispatch.go` now gives the wrappers a checked C-width dispatch seam while retaining scalar kernels. `TestH264LoopFilterDispatchMatchesScalar` and `TestH264LoopFilterHighDispatchMatchesScalar` cover luma/chroma, intra/non-intra, vertical/horizontal, MBAFF, and chroma 4:2:2 shapes before amd64/arm64 asm is enabled. |
