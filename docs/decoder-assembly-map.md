...
| --- | --- | --- | --- | --- |
| 1 | Qpel 8-bit size 16/8 all positions | partial | partial | Horizontal row leaves `mc00`/`mc10`/`mc20`/`mc30` and vertical row leaves `mc01`/`mc02`/`mc03` put/avg for sizes 16/8 are implemented in `internal/h264/qpel_mc_amd64.s` and `internal/h264/qpel_mc_arm64.s`; `mc00` also covers sizes 4/2. Remaining leaves: diagonal and HV qpel positions, plus smaller fractional sizes. Kernel seam in `internal/h264/qpel.go` uses `[]uint8`, native signed pointer-width strides/offsets, and 32-bit C-int selectors. Verify against `TestH264Qpel*`, `qpel_oracle_test.go`, and real-vector `caba3`/`canl4`. |
