...
partial asm around 106.8 ms/sample versus about 141.7 ms/sample for `purego`.
...
| --- | --- | --- | --- | --- |
| 1 | Loop filter 8-bit luma/chroma SIMD | partial | partial | arm64 now mirrors FFmpeg `ff_h264_v/h_loop_filter_luma_neon`, `ff_h264_v/h_loop_filter_chroma_neon`, `ff_h264_h_loop_filter_chroma422_neon`, and `ff_h264_v/h_loop_filter_luma_intra_neon` with raw NEON instruction words behind Go ABI wrappers. The focused luma deblock benchmark improved from roughly 55-57 ns/op purego to about 11 ns/op vertical and 18-21 ns/op horizontal on Apple arm64, with 0 allocs/op. Focused chroma deblock medians are about 9.2 ns/op vertical and 14.4 ns/op horizontal in default builds versus about 18.4 and 18.5 ns/op in `purego`, the focused 4:2:2 horizontal row is about 20.6 ns/op default versus 33.9 ns/op `purego`, focused luma intra vertical is about 10.0 ns/op default versus 57.8 ns/op `purego`, and focused luma intra horizontal is about 17.4 ns/op default versus 57.8 ns/op `purego`, all 0 allocs/op. The 8-bit high-422 `frext-hi422fr10-sony-b` real-vector benchmark passes raw-MD5 and the Go zero-allocation gate in both default and `purego` lanes, with default partial asm about 224.9 ms/sample versus about 266.6 ms/sample for `purego` in the sampled run. The `caba3` real-vector benchmark also passes after the luma intra ports, with default partial asm about 106.8 ms/sample versus about 141.7 ms/sample for `purego` in the sampled run. amd64 still needs real FFmpeg `ff_deblock_v_luma_8_*`/`ff_deblock_h_luma_8_*` SSE2/AVX math instead of the current scalar-equivalent leaf, plus chroma leaves. ABI remains `*uint8` pixels, native signed `int` stride, `int32` alpha/beta, and `*int8` tc0 where the upstream prototype has tc0. |
| 2 | Loop filter intra/MBAFF/high | todo | partial | Cover chroma intra, `ff_deblock_h_luma_mbaff_8_*`, and high-bit leaves. Keep high-bit assembly boundaries byte-pointer/byte-stride shaped where mirroring FFmpeg. |
