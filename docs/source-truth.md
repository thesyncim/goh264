...
pictures, bounded exact luma-DC, chroma-only, and combined luma/chroma
residual-P admission, Annex B/AVC access-unit output, configured multi-slice
output, and RTP packetization modes 0 and 1.
...
bounded luma-DC, chroma-only, and combined luma/chroma residual-P admission
across Annex B/configured AVC/RTP plus mode-0 packetization with local and
FFmpeg rawvideo decode proof,
...
non-RTP `DONDisabled` normalization for later RTP re-entry while direct RTP
interleaved-DON configs reject,
...
including checked clipped direct byte views, append-source, and destination
validation,
...
overflowed bitrate budget derivation rejection, zero/negative/max-int VBV byte
budget boundary conversion, overflowed input-plane geometry rejection, and P-skip
...
explicit max-frame-size, slice-byte, encode-time, preset, slice-count,
SPS/PPS-before-IDR, and disabled-state intra-refresh setters with zero-valued
runtime budget disablement and unsupported enablement preservation,
...
NAL type is another packetization unit, payload byte parity against the
access-unit NAL list, frame-level FU-A start/continuation/end sequence
validation, and packet storage isolated from
...
residual-P admission across all 4x4 luma blocks plus chroma-only and combined
luma/chroma residual-P for Annex B, AVC, RTP mode 1, and RTP mode 0, while
broader residual-shaped P-frame deltas remain on the recovery-SEI plus P
IntraPCM fallback boundary.
